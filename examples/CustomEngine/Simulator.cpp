...
cSimulator::cSimulator(const std::string& config_path)
{
...
void cSimulator::AddObj(int n, const std::string& obj_type, bool add_perturb /*=false*/)
{
...
void cSimulator::AddMultibody(const std::string& skeleton_name)
{
...
void cSimulator::StepSimulation(double dt)
{
	...
	{
		// get the jacobian directly and exit
		if (mMultibody)
		{
			// std::cout << "[rec] q=" << mMultibody->Getq().transpose() << std::endl;
			// std::cout << "[rec] qdot=" << mMultibody->Getqdot().transpose() << std::endl;
			// tMatrixXd jac;

			// // tVector local_pos = tVector(0, 0, 0, 1);
			// tVector local_pos = tVector(1, 1, 1, 1);
			// tVector3d global_pos = (mMultibody->GetLinkById(0)->GetGlobalTransform() * local_pos).segment(0, 3);
			// mMultibody->ComputeJacobiByGivenPointTotalDOFWorldFrame(0, global_pos, jac);

			// auto link = mMultibody->GetLinkById(0);

			// std::cout << "link 0 "
			// 		  << "pos = " << link->GetWorldPos().transpose() << std::endl;
			// std::cout << "link 0 "
			// 		  << "trans = \n " << link->GetGlobalTransform() << std::endl;
			...
			if (global_frame_id == 0)
			{
				...
				// OutputJacobianTestJson(mMultibody, "test_jac.rec");
				// OutputDynamicsFTestJson(mMultibody, "test_dyna.rec");
			}
...
void cSimulator::PostUpdate(double dt)
{
	mTime += dt;
}
...
void cSimulator::createRigidBody(double mass, const btTransform& startTransform, btCollisionShape* shape, const std::string& name, const btVector4& color)
{
...
#include "Model/EulerAngelRotationMatrix.h"
// 1. Apply active force, update the velocity, write into bullet
void cSimulator::ApplyActiveForce(double dt)
{
...
void cSimulator::CollisionRespose(double dt)
{
	// no collision, return
...
void cSimulator::CollisionResposePenalty(double dt)
{
	const double k = 2000;
...
void cSimulator::CollisionResposeLCP(double dt)
{
...
void cSimulator::CollisionResposeSI(double dt)
{
}

// 4. update transform, semi-implicit
// void cSimulator::UpdateTransform(double dt)
// {
// 	for (auto& obj : mSimObjs)
// 	{
...
void cSimulator::UpdateVelocityInternal(double dt)
{
	for (auto& obj : mSimObjs)
...
void cSimulator::UpdateVelocityInternalWithoutCoriolis(double dt)
{
	for (auto& obj : mSimObjs)
		obj->UpdateVelocity(dt);
	if (mMultibody)
...
void cSimulator::Update(double dt)
{
...
void cSimulator::PushStatePreCollision()
{
...
void cSimulator::PopStatePostColliison()
{
	// std::cout << "here, only push & pop velocity and force state\n";
	std::string name = "pre_collision";
	for (auto& obj : mSimObjs)
...
void cSimulator::CollectFrameInfo(double dt)
{
	tFrameInfo info;
...
void cSimulator::WriteFrameInfo(const std::string& path)
{
	Json::Value root;
