...
tMatrix(*RotFunc4d[3])(double) = { xconventionTransform, yconventionTransform, zconventionTransform };
tMatrix3d(*RotFunc3d[3])(double) = { xconventionRotation, yconventionRotation, zconventionRotation };
...
tVector3d XYZtoYZX(double x, double y, double z) {
	tMatrix3d m = zconventionRotation(z) * yconventionRotation(y) * xconventionRotation(x);
...
tMatrix3d EulerAngleRotation(const tVector3d & v, const Eigen::Vector3i & order) {
	...
	//return zconventionRotation(v[0]) * yconventionRotation(v[1]) * xconventionRotation(v[0]);
	//return zconventionRotation(v[order[0]]) * yconventionRotation(v[order[1]]) * xconventionRotation(v[order[0]]);
...
tVector3d RotMatToAxisAngle(const tMatrix3d & R) {
	...
	for(int i = 0; i < 3; ++i) {
		if (abs(e.coeff(i) - 1.) < 1e-6) {
			...
			tVector3d v;
			if (u[0] != 0) v = tVector3d(-(u[1] + u[2]) / u[0], 1., 1.);
			else if (u[1] != 0) v = tVector3d(1., -(u[0] + u[2]) / u[1], 1.);
			else if (u[2] != 0)  v = tVector3d(1., 1., -(u[0] + u[1]) / u[2]);
			else {
				return { 0, 0, 0 };
			}
...
tMatrix EulerAngleTransform(const tVector3d & v, const Eigen::Vector3i & order) {
	return RotFunc4d[order[0]](v[order[0]]) * RotFunc4d[order[1]](v[order[1]]) * RotFunc4d[order[2]](v[order[2]]);
}
