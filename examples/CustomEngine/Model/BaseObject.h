...
const std::string shape_type_keys[TOTAL_SHAPE_TYPE] = {
	"sphere",
	"box",
	"capsule",
	"cylinder"};
...
struct BaseObjectShapeParam
{
	...
	double width;
	double height;
	BaseObjectShapeParam() : shape_type(0), top_scale(1.), bottom_scale(1.), length(0), width(0), height(0) {}
};
...
public:
	...
	virtual void ComputeJacobian(const tVector3d& end_pos, tMatrix& m, int method) {}
	...
	Mesh* GetMesh() const;
	const tMatrix& GetModelMatrix() const;
	const tVector3d& GetWorldPos();
	virtual int GetNumOfFreedom();
	virtual Freedom* GetFreedoms(int order) { return nullptr; }
	void SetParentJoint(BaseObject* parent_joint) { this->parent_joint = parent_joint; }
	BaseObject* GetChild(int order);
	virtual void SetFreedomValue(int id, double v) {}
	virtual void GetFreedomValue(int id, double& v) {}
	virtual Freedom* GetFreedomByAxis(tVector3d axis, int type = REVOLUTE) { return nullptr; }
	...
	void SetNumTotalFreedoms(int total_freedoms) { this->total_freedoms = total_freedoms; }
	void GetGlobalTransform(tMatrix& m) const { m = global_transform; };
	const tMatrix& GetGlobalTransform() { return global_transform; };
	void GetLocalTransform(tMatrix& m) const { m = local_transform; };
	const tMatrix& GetLocalTransform() { return local_transform; }
	...
	const tMatrix& GetNegInitRotationtMatrix() const { return neg_init_rotation_matrix_4x4; };
	...
	void SetComputeSecondDerive(bool flag) { this->compute_second_derive = flag; }
...
protected:
	...
	BaseObject* parent_joint;  // For joint, this variable is the parent of the parent link, for link it is jut the parent
