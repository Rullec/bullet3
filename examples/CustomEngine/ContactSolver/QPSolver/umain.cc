/*
  ...
  This file contains just an example on how to set-up the matrices for using
with the solve_quadprog() function.
...
int main(int argc, char *const argv[])
{
    ...
    clock.restart();
    std::cout << "f: "
              << uQuadProgPP::solve_quadprog(G, g0, CE, ce0, CI, ci0, x)
              << std::endl;
    ...
    clock.restart();
    std::cout << QP::solve_quadprog(eG, eg0, eCe, ece, eCi, eci, ex)
              << std::endl;
