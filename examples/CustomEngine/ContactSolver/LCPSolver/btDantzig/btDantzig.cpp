...
cBulletDantzigLCPSolver::cBulletDantzigLCPSolver() : cLCPSolverBase(eLCPSolverType::BulletDantzig)
{
...
*/
int cBulletDantzigLCPSolver::Solve(int num_of_vars, const tMatrixXd& M, const tVectorXd& n, tVectorXd& x)
{
	// Test();
	...
	btAlignedObjectArray<int> limitDependency;
	limitDependency.resize(num_of_vars);
	
	bool useSparsity = false;
	bool succ = solver->solveMLCP(M_buf, n_buf, x_buf, lo_buf, hi_buf, limitDependency, 1000, useSparsity);
...
void cBulletDantzigLCPSolver::Test()
{
	// 	int num_of_vars = 10;
	// 	tMatrixXd M = tMatrixXd::Zero(num_of_vars, num_of_vars);
	// 	tVectorXd n = tVectorXd::Zero(num_of_vars);
