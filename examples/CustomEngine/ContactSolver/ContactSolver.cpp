...
#include "LCPSolver/LCPSolverBuilder.hpp"
...
cContactSolver::cContactSolver(const std::string& config_path, btDiscreteDynamicsWorld* world)
{
	...
	{
		std::cout << "ContactSolver: Use LCP and SI config at the same time, illegal\n";
		err = true;
	}
	...
	if (mEnableSILCPComparision == true)
	{
		...
		{
			std::cout << "SI & LCP comparision is enabled but they are not calculated both in advance\n";
			err = true;
		}
	}
	if (err) exit(1);
...
void cContactSolver::ConstraintFinished()
{
	...
	// std::cout << "[debug] result constraint torque 0 = " << contact_torque_array[0]->joint_torque.transpose() << std::endl;
	DeleteColObjData();
...
void cContactSolver::ConstraintSetup()
{
	...
	if (mNumContactPoints + mNumJointLimitConstraints != 0)
	{
		...
		// This convert matrix are shared by LCP and SI
		CalcAbsvelConvertMat();

		if (mEnableConvertMatTest) TestCartesianForceToCartesianVel();
		// exit(1);
		// the following calculation can only used in LCP solver, discard when LCP is disabled
		if (mEnableLCPCalc)
		{
			...
			// 	if (mEnableContactLCP)
			// 	{
			// 		fout.open("contact_enable.txt");
			// 		fout << "convert mat = \n"
			// 			 << mColGroupData[0]->mConvertCartesianForceToVelocityMat << std::endl;
			// 		fout << "convert vec = "
			// 			 << mColGroupData[0]->mConvertCartesianForceToVelocityVec.transpose() << std::endl;
			// 		fout.close();
			// 	}
			// 	else
			// 	{
			// 		fout.open("contact_disable.txt");
			// 		fout << "convert mat = \n"
			// 			 << mColGroupData[0]->mConvertCartesianForceToVelocityMat << std::endl;
			// 		fout << "convert vec = "
			// 			 << mColGroupData[0]->mConvertCartesianForceToVelocityVec.transpose() << std::endl;
			// 	}
			// }
			if (mEnableConvertMatTest) TestCartesianForceToCartesianRelVel(rel_vel_convert_mat, rel_vel_convert_vec);
			...
			// 	{
			// 		fout.open("joint_enable.txt");
			// 		fout << "normal convert mat = \n"
			// 			 << normal_vel_convert_mat << std::endl;
			// 		fout << "normal convert vec = "
			// 			 << normal_vel_convert_vec.transpose() << std::endl;
			// 		fout << "tangent convert mat = \n"
			// 			 << tangent_vel_convert_mat << std::endl;
			// 		fout << "tangent convert vec = "
			// 			 << tangent_vel_convert_vec.transpose() << std::endl;
			// 		fout.close();
			// 	}
			// 	else
			// 	{
			// 		fout.open("joint_disable.txt");
			// 		fout << "normal convert mat = \n"
			// 			 << normal_vel_convert_mat << std::endl;
			// 		fout << "normal convert vec = "
			// 			 << normal_vel_convert_vec.transpose() << std::endl;
			...
			// 	}
			// }
			if (mEnableConvertMatTest) TestCartesianForceToNormalAndTangetRelVel(normal_vel_convert_mat,
																				 normal_vel_convert_vec,
																				 tangent_vel_convert_mat,
			...
			if (mEnableConvertMatTest) TestCartesianForceToNormalAndTangetResultBasedRelVel(normal_vel_convert_result_based_mat,
...
void cContactSolver::SolveByLCP()
{
	...
	int final_shape = mNumContactPoints * single_size + mNumJointLimitConstraints;
	...
	for (int i = 0; i < mNumContactPoints; i++)
	{
		...
		if (mEnableFrictionalLCP)
		{
			line_M.block(1, 0, mNumFrictionDirs, final_shape) = tangent_vel_convert_result_based_mat.block(i * mNumFrictionDirs, 0, mNumFrictionDirs, final_shape);
	...
	for (int i = 0; i < mNumJointLimitConstraints; i++)
	{
		M.block(mNumContactPoints * single_size + i, 0, 1, final_shape) = normal_vel_convert_result_based_mat.block(mNumContactPoints + i, 0, 1, final_shape);
		n[mNumContactPoints * single_size + i] = normal_vel_convert_result_based_vec[mNumContactPoints + i];
	}
	// std::cout << "[lcp] LCP M cond num = " << cMathUtil::CalcConditionNumber(M) << std::endl;
	// add small values to diagnal to keep it away from singular, similar to cfm varaible in ODE
	// as RTQL8 said
	// std::cout << "[debug] add diagnoal eps " << mDiagonalEps << " on LCP M\n";
	for (int i = 0; i < M.rows(); i++)
	{
		M(i, i) *= (1 + mDiagonalEps);
	}
	// M += tMatrixXd::Identity(M.rows(), M.cols()) * 1e-5;
	// std::cout << "[lcp] After M cond num = " << cMathUtil::CalcConditionNumber(M) << std::endl;
	// std::cout << "begin to do lcp solveing\n";
	// std::ofstream fout(log_path, std::ios::app);
	// fout << "----------------frame " << global_frame_id << std::endl;
	// fout << "q = " << mMultibodyArray[0]->Getq().transpose() << std::endl;
	// fout << "qdot = " << mMultibodyArray[0]->Getqdot().transpose() << std::endl;
	// fout << "M = \n"
	// 	 << M << std::endl;
	...
	// std::cout << "[lcp] q = " << mMultibodyArray[0]->Getq().transpose() << std::endl;
	// std::cout << "[lcp] qdot = " << mMultibodyArray[0]->Getqdot().transpose() << std::endl;
	// exit(1);
...
void cContactSolver::ConvertLCPResult()
{
	...
	for (int i = 0; i < mNumContactPoints; i++)
	{
		...
		{
			std::cout << "[lcp] num contact pt = " << mNumContactPoints << std::endl;
	...
	{
		auto& data = mJointLimitConstraintData[i];
		double generalized_force = x_lcp[mNumContactPoints * single_contact_size + i];
...
/**
 * \brief			Get all collision object in this world and create the collision group
	...
	for (int i = 0; i < n_objs; i++)
	{
		...
		switch (obj->GetType())
		{
			...
			case eColObjType::RobotCollder:
			{
				...
				std::map<cRobotModelDynamics*, int>::iterator it = model_set.find(model);
				if (it == model_set.end())
				{
...
void cContactSolver::CalcAbsvelConvertMat()
{
	...
	for (int i = 0; i < mColGroupData.size(); i++)
	{
		...
		data->Setup(mContactConstraintData.size(), mJointLimitConstraintData.size());
...
void cContactSolver::CalcRelvelConvertMat()
{
	...
	for (int i = 0; i < mNumContactPoints; i++)
	{
		...
		if (data->mIsSelfCollision == false)
		{
			...
			rel_vel_convert_mat.block(i * 3, 0, 3, final_shape) =
				mColGroupData[body0_groupid]->mConvertCartesianForceToVelocityMat.block(i * 3, 0, 3, final_shape);
			if (mColGroupData[body1_groupid]->mBody->IsStatic() == false)
				rel_vel_convert_mat.block(i * 3, 0, 3, final_shape) -= mColGroupData[body1_groupid]->mConvertCartesianForceToVelocityMat.block(i * 3, 0, 3, final_shape);
		...
		else
		{
			...
			rel_vel_convert_mat.block(i * 3, 0, 3, final_shape) =
				mColGroupData[single_groupid]->mConvertCartesianForceToVelocityMat.block(i * 3, 0, 3, final_shape);
	...
	for (int i = 0; i < mNumJointLimitConstraints; i++)
	{
		...
		int group_id = map_colobjid_to_groupid[data->multibody->GetLinkCollider(0)->getWorldArrayIndex()];

		rel_vel_convert_mat.block(row_st, 0, 1, final_shape) = mColGroupData[group_id]->mConvertCartesianForceToVelocityMat.block(row_st, 0, 1, final_shape);
		rel_vel_convert_vec.segment(row_st, 1) = mColGroupData[group_id]->mConvertCartesianForceToVelocityVec.segment(row_st, 1);
	}
...
void cContactSolver::CalcDecomposedConvertMat()
{
	...
	// }
	// int n_contact = mContactConstraintData.size();
	int x_size = 3 * mNumContactPoints + mNumJointLimitConstraints;  // x is the collection of contact force
	if (x_size == 0) return;

	// the normal vel of one contact point or one joint limit constraint occupy a single line
	normal_vel_convert_mat.resize(mNumContactPoints + mNumJointLimitConstraints, x_size);  // one contact point one line
	normal_vel_convert_vec.resize(mNumContactPoints + mNumJointLimitConstraints);          // one contact point one real number

	if (mEnableFrictionalLCP)
	{
		tangent_vel_convert_mat.resize(mNumFrictionDirs * mNumContactPoints, x_size);  // one contact point N lines, N = mNumFrictionDir
		tangent_vel_convert_vec.resize(mNumFrictionDirs * mNumContactPoints);          // one contact point one real number
	}
	...
	for (int i = 0; i < mNumContactPoints; i++)
	{
		...
		if (mEnableFrictionalLCP)
		{
			...
			tangent_vel_convert_mat.block(mNumFrictionDirs * i, 0, mNumFrictionDirs, x_size) =
	...
	normal_vel_convert_mat.block(mNumContactPoints, 0, mNumJointLimitConstraints, x_size) = rel_vel_convert_mat.block(mNumContactPoints * 3, 0, mNumJointLimitConstraints, x_size);
	normal_vel_convert_vec.segment(mNumContactPoints, mNumJointLimitConstraints) = rel_vel_convert_vec.segment(mNumContactPoints * 3, mNumJointLimitConstraints);
}

/**
 * \brief				Add a contact "manifold" into the constraint solver.
 * 
...
void cContactSolver::AddManifold(btPersistentManifold* manifold)
{
	...
	for (int j = 0; j < n_contacts; j++)
	{
		...
		int col_group0_data_main_group = map_colobjid_to_groupid[mColGroupData[data->mbody0GroupId]->mBody->getWorldArrayIndex()];
		int col_group1_data_main_group = map_colobjid_to_groupid[mColGroupData[data->mbody1GroupId]->mBody->getWorldArrayIndex()];
		...
		else
		{
			if (mEnableMultibodySelfCol == true)
			{
				if (mEnableDebugOutput)
				{
					std::cout << "[log] add self collision contact point into ColGroup" << col_group1_data_main_group << ", " << data->mBodyA->GetName() << " and " << data->mBodyB->GetName() << std::endl;
				}
...
void cContactSolver::AddJointLimit()
{
	...
	for (int mb_id = 0; mb_id < mMultibodyArray.size(); mb_id++)
	{
		...
		// only care the revolute and spherical joint, no root joint, no prismatic joint

		// 2. judge for all dofs besides root joint
		int root_dof_end = model->GetJointById(0)->GetNumOfFreedom();
		for (int dof_id = root_dof_end; dof_id < model->GetNumOfFreedom(); dof_id++)
		{
			...
			if (q[dof_id] >= ub[dof_id] || q[dof_id] <= lb[dof_id])
			{
				...
				int group_id = map_colobjid_to_groupid[model->GetLinkCollider(0)->getWorldArrayIndex()];
				mColGroupData[group_id]->AddJointLimitConstraint(mJointLimitConstraintData.back());
				mNumJointLimitConstraints++;
				if (mEnableDebugOutput)
				{
					std::cout << "[joint limit] add limit for dof " << dof_id << " val = " << q[dof_id] << ", is upper bound = " << upper_bound_violate << std::endl;
				}
...
void cContactSolver::CalcResultVectorBasedConvertMat()
{
	...
	if (mEnableFrictionalLCP)
		single_size += mNumFrictionDirs + 1;
	int result_vector_size = mNumContactPoints * single_size + mNumJointLimitConstraints;
	normal_vel_convert_result_based_mat.resize(mNumContactPoints + mNumJointLimitConstraints, result_vector_size);
	normal_vel_convert_result_based_mat.setZero();
	normal_vel_convert_result_based_vec.resize(mNumContactPoints + mNumJointLimitConstraints);
	if (mEnableFrictionalLCP)
	{
		tangent_vel_convert_result_based_mat.resize(mNumContactPoints * mNumFrictionDirs, result_vector_size);
		tangent_vel_convert_result_based_mat.setZero();
		tangent_vel_convert_result_based_vec.resize(mNumContactPoints * mNumFrictionDirs);
	...
	for (int i = 0; i < mNumContactPoints; i++)
	{
		for (int j = 0; j < mNumContactPoints; j++)
		{
			...
			{
				// frictional case
				normal_vel_convert_result_based_mat.block(i, j * single_size, 1, single_size) = normal_vel_convert_mat.block(i, j * 3, 1, 3) * data->mS;

				tangent_vel_convert_result_based_mat.block(i * mNumFrictionDirs, j * single_size, mNumFrictionDirs, single_size) =
					tangent_vel_convert_mat.block(i * mNumFrictionDirs, j * 3, mNumFrictionDirs, 3) * data->mS;
			}
			else
			{
				// only normal case
				normal_vel_convert_result_based_mat.block(i, j * single_size, 1, single_size) = normal_vel_convert_mat.block(i, j * 3, 1, 3) * data->mNormalPointToA.segment(0, 3);
			}
		}
		...
		{
			normal_vel_convert_result_based_mat.block(i, mNumContactPoints * single_size + j, 1, 1) = normal_vel_convert_mat.block(i, mNumContactPoints * 3 + j, 1, 1);
			if (mEnableFrictionalLCP)
			{
				tangent_vel_convert_result_based_mat.block(i * mNumFrictionDirs, mNumContactPoints * single_size + j, mNumFrictionDirs, 1) = tangent_vel_convert_mat.block(i * mNumFrictionDirs, mNumContactPoints * 3 + j, mNumFrictionDirs, 1);
			}
		}
	}
	...
	for (int i = 0; i < mNumJointLimitConstraints; i++)
	{
		...
		{
			auto& data = mContactConstraintData[j];
			normal_vel_convert_result_based_mat.block(row_st + i, j * single_size, 1, single_size) = normal_vel_convert_mat.block(row_st + i, j * 3, 1, 3) * data->mS;
		}

		for (int j = 0; j < mNumJointLimitConstraints; j++)
		{
			auto& data = mContactConstraintData[j];
			normal_vel_convert_result_based_mat.block(row_st + i, mNumContactPoints * single_size + j, 1, 1) = normal_vel_convert_mat.block(row_st + i, mNumContactPoints * 3 + j, 1, 1);
		}
	}
