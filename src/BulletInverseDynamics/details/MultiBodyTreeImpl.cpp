...
const char *
MultiBodyTree::MultiBodyImpl::jointTypeToString(const JointType &type) const
{
    switch (type)
    {
    case FIXED:
...
void MultiBodyTree::MultiBodyImpl::printTree()
{
    id_printf("body %.2d[%s]: root\n", 0,
              jointTypeToString(m_body_list[0].m_joint_type));
    printTree(0, 0);
}
...
void MultiBodyTree::MultiBodyImpl::printTreeData()
{
    for (idArrayIdx i = 0; i < m_body_list.size(); i++)
    {
        ...
        id_printf("q_index= %d\n", body.m_q_index);
        id_printf("Jac_JR= [%f;%f;%f]\n", body.m_Jac_JR(0), body.m_Jac_JR(1),
                  body.m_Jac_JR(2));
        id_printf("Jac_JT= [%f;%f;%f]\n", body.m_Jac_JT(0), body.m_Jac_JT(1),
                  body.m_Jac_JT(2));

        id_printf("mass = %f\n", body.m_mass);
        id_printf("mass * com = [%f %f %f]\n", body.m_body_mass_com(0),
                  body.m_body_mass_com(1), body.m_body_mass_com(2));
        id_printf("I_o= [%f %f %f;\n"
                  "	  %f %f %f;\n"
                  "	  %f %f %f]\n",
                  body.m_body_I_body(0, 0), body.m_body_I_body(0, 1),
                  body.m_body_I_body(0, 2), body.m_body_I_body(1, 0),
                  body.m_body_I_body(1, 1), body.m_body_I_body(1, 2),
                  body.m_body_I_body(2, 0), body.m_body_I_body(2, 1),
                  body.m_body_I_body(2, 2));

        id_printf("parent_pos_parent_body_ref= [%f %f %f]\n",
                  body.m_parent_pos_parent_body_ref(0),
                  body.m_parent_pos_parent_body_ref(1),
                  body.m_parent_pos_parent_body_ref(2));
    }
}
...
void MultiBodyTree::MultiBodyImpl::printTree(int index, int indentation)
{
    ...
    for (int i = 0; i < num_children; i++)
    {
        ...
        id_printf("body %.2d[%s]: %.2d is child no. %d (qi= %d .. %d) \n",
                  index, jointTypeToString(m_body_list[index].m_joint_type),
                  child_index, (count++) + 1, m_body_list[index].m_q_index,
                  m_body_list[index].m_q_index +
                      bodyNumDoFs(m_body_list[index].m_joint_type));
        // first grandchild
...
int MultiBodyTree::MultiBodyImpl::generateIndexSets()
{
    ...
    for (idArrayIdx child = 1; child < m_parent_index.size(); child++)
    {
        ...
        if (parent >= 0 &&
            parent < (static_cast<int>(m_parent_index.size()) - 1))
        {
        ...
        else
        {
            if (-1 == parent)
            {
                // multiple bodies are directly linked to the environment, ie,
                // not a single root
                bt_id_error_message(
            ...
            {
                // should never happen
                bt_id_error_message("building index sets. parent_index[%zu]= "
                                    "%d, but m_parent_index.size()= %d\n",
                                    child, parent,
                                    static_cast<int>(m_parent_index.size()));
            }
            return -1;
        }
    }
...
void MultiBodyTree::MultiBodyImpl::calculateStaticData()
{
    ...
    for (idArrayIdx i = 0; i < m_body_list.size(); i++)
    {
        ...
        case PRISMATIC:
            ...
            body.m_parent_Jac_JT =
                body.m_body_T_parent_ref.transpose() * body.m_Jac_JT;
...
int MultiBodyTree::MultiBodyImpl::calculateInverseDynamics(const vecx &q,
                                                           const vecx &u,
                                                           const vecx &dot_u,
                                                           vecx *joint_forces)
{
    if (q.size() != m_num_dofs || u.size() != m_num_dofs ||
        dot_u.size() != m_num_dofs || joint_forces->size() != m_num_dofs)
    {
        bt_id_error_message("wrong vector dimension. system has %d DOFs,\n"
                            "but dim(q)= %d, dim(u)= %d, dim(dot_u)= %d, "
                            "dim(joint_forces)= %d\n",
                            m_num_dofs, static_cast<int>(q.size()),
                            static_cast<int>(u.size()),
                            static_cast<int>(dot_u.size()),
                            static_cast<int>(joint_forces->size()));
        return -1;
    }
    ...
    {
        RigidBody &body = m_body_list[i];
        // 3.4 update dynamic terms (rate of change of angular & linear
        // momentum)
        body.m_eom_lhs_rotational =
            body.m_body_I_body * body.m_body_ang_acc +
            body.m_body_mass_com.cross(body.m_body_acc) +
            body.m_body_ang_vel.cross(body.m_body_I_body *
                                      body.m_body_ang_vel) -
            body.m_body_moment_user;
        {
            // std::cout << "[debug] body.m_body_ang_acc = " <<
            // btBulletUtil::btIDVectorTotVector0(body.m_body_ang_acc).transpose()
            // << std::endl; std::cout << "[debug] body.m_body_mass_com = " <<
            // btBulletUtil::btIDVectorTotVector0(body.m_body_mass_com).transpose()
            // << std::endl; std::cout << "[debug] body.m_body_acc = " <<
            // btBulletUtil::btIDVectorTotVector0(body.m_body_acc).transpose()
            // << std::endl; std::cout << "[debug] body.m_body_ang_vel = " <<
            // btBulletUtil::btIDVectorTotVector0(body.m_body_ang_vel).transpose()
            // << std::endl; std::cout << "[debug] body.m_body_I_body = " <<
            // btBulletUtil::btIDMatrixTotMatrix(body.m_body_I_body).transpose()
            // << std::endl; std::cout << "[debug] body.m_body_moment_user = "
            // <<
            // btBulletUtil::btIDVectorTotVector0(body.m_body_moment_user).transpose()
            // << std::endl; std::cout << "[debug] body.m_eom_lhs_rotational = "
            // <<
            // btBulletUtil::btIDVectorTotVector0(body.m_eom_lhs_rotational).transpose()
            // << std::endl;

            // std::cout << "[debug] term1 = " <<
            // btBulletUtil::btIDVectorTotVector0(body.m_body_I_body *
            // body.m_body_ang_acc).transpose() << std::endl; std::cout <<
            // "[debug] term2 = " <<
            // btBulletUtil::btIDVectorTotVector0(body.m_body_mass_com.cross(body.m_body_acc)).transpose()
            // << std::endl; std::cout << "[debug] term3 = " <<
            // btBulletUtil::btIDVectorTotVector0(body.m_body_ang_vel.cross(body.m_body_I_body
            // * body.m_body_ang_vel)).transpose() << std::endl; std::cout <<
            // "[debug] term4 = " <<
            // btBulletUtil::btIDVectorTotVector0(body.m_body_moment_user).transpose()
            // << std::endl;
        }
        body.m_eom_lhs_translational =
            body.m_body_ang_acc.cross(body.m_body_mass_com) +
            body.m_mass * body.m_body_acc +
            body.m_body_ang_vel.cross(
                body.m_body_ang_vel.cross(body.m_body_mass_com)) -
    ...
    // Also, this enables adding zero weight bodies as a way to calculate frame
    // poses for force elements, etc.
    ...
    for (int body_idx = m_body_list.size() - 1; body_idx >= 0; body_idx--)
    {
        ...
        for (idArrayIdx child_list_idx = 0;
             child_list_idx < m_child_indices[body_idx].size();
             child_list_idx++)
        {
            const RigidBody &child =
                m_body_list[m_child_indices[body_idx][child_list_idx]];
            ...
            sum_m_children -=
                child.m_body_T_parent.transpose() * child.m_moment_at_joint +
                child.m_parent_pos_parent_body.cross(
                    child_joint_force_in_this_frame);
        }
    ...
    for (idArrayIdx i = 0; i < m_body_revolute_list.size(); i++)
    {
        ...
        // (*joint_forces)(body.m_q_index) = body.m_Jac_JR.transpose() *
        // body.m_moment_at_joint;
        (*joint_forces)(body.m_q_index) =
            body.m_Jac_JR.dot(body.m_moment_at_joint);
    }
    ...
    for (idArrayIdx i = 0; i < m_body_prismatic_list.size(); i++)
    {
        ...
        // (*joint_forces)(body.m_q_index) = body.m_Jac_JT.transpose() *
        // body.m_force_at_joint;
        (*joint_forces)(body.m_q_index) =
            body.m_Jac_JT.dot(body.m_force_at_joint);
    }
...
int MultiBodyTree::MultiBodyImpl::calculateKinematics(const vecx &q,
                                                      const vecx &u,
                                                      const vecx &dot_u,
                                                      const KinUpdateType type)
{
    // std::cout << "q = "<< btBulletUtil::btIDArrayToEigenArray(q).transpose()
    // << std::endl; std::cout << "u = "<<
    // btBulletUtil::btIDArrayToEigenArray(u).transpose() << std::endl;
    // std::cout << "udot = "<<
    // btBulletUtil::btIDArrayToEigenArray(dot_u).transpose() << std::endl;
    if (q.size() != m_num_dofs || u.size() != m_num_dofs ||
        dot_u.size() != m_num_dofs)
    {
        bt_id_error_message("wrong vector dimension. system has %d DOFs,\n"
                            "but dim(q)= %d, dim(u)= %d, dim(dot_u)= %d\n",
                            m_num_dofs, static_cast<int>(q.size()),
    ...
    if (type != POSITION_ONLY && type != POSITION_VELOCITY &&
        type != POSITION_VELOCITY_ACCELERATION)
    {
    ...
    for (idArrayIdx i = 0; i < m_body_revolute_list.size(); i++)
    {
        ...
        // std::cout << "[debug] revolute jac jr = " <<
        // btBulletUtil::btIDVectorTotVector0(body.m_Jac_JR).transpose() <<
        // std::endl; std::cout << "[debug] revolute parent to body = \n" <<
        // btBulletUtil::btIDMatrixTotMatrix(body.m_body_T_parent) << std::endl;
        if (type >= POSITION_VELOCITY)
        {
            ...
            // \n" <<
            // btBulletUtil::btIDVectorTotVector0(body.m_body_ang_vel_rel).transpose()
            // << std::endl;
        }
        if (type >= POSITION_VELOCITY_ACCELERATION)
        {
            ...
            // \n" <<
            // btBulletUtil::btIDVectorTotVector0(body.m_body_ang_acc_rel).transpose()
            // << std::endl;
        }
    }
    ...
    for (idArrayIdx i = 0; i < m_body_prismatic_list.size(); i++)
    {
        ...
        body.m_parent_pos_parent_body =
            body.m_parent_pos_parent_body_ref +
            body.m_parent_Jac_JT * q(body.m_q_index);
        if (type >= POSITION_VELOCITY)
        {
            body.m_parent_vel_rel = body.m_body_T_parent_ref.transpose() *
                                    body.m_Jac_JT * u(body.m_q_index);
        }
        if (type >= POSITION_VELOCITY_ACCELERATION)
        {
            body.m_parent_acc_rel =
                body.m_parent_Jac_JT * dot_u(body.m_q_index);
        }
    }
    ...
    for (idArrayIdx i = 0; i < m_body_floating_list.size(); i++)
    {
        ...
        // btBulletUtil::btIDVectorTotVector0(body.m_parent_pos_parent_body).transpose()
        // << std::endl;
        ...
        if (type >= POSITION_VELOCITY)
        {
            ...
            // body.m_parent_vel_rel = body.m_body_T_parent.transpose() *
            ...
            // ת����body����ϵ
            // std::cout << "[debug] root ang vel in world frame = " <<
            // btBulletUtil::btVectorTotVector0(body.m_body_ang_vel_rel).transpose()
            // << std::endl; in parent frame
            body.m_body_ang_vel_rel =
                body.m_body_T_parent * body.m_body_ang_vel_rel;
            // std::cout << "[debug] root ang vel in body frame = " <<
            // btBulletUtil::btVectorTotVector0(body.m_body_ang_vel_rel).transpose()
            // << std::endl;
            ...
            body.m_parent_vel_rel(1) = u(body.m_q_index + 4);
            body.m_parent_vel_rel(2) = u(body.m_q_index + 5);
            // std::cout << "[debug] root lin vel in world frame = " <<
            // btBulletUtil::btVectorTotVector0(body.m_parent_vel_rel).transpose()
            // << std::endl; std::cout << "[debug] body to parent rot mat = \n"
            // <<
            // btBulletUtil::btIDMatrixTotMatrix(body.m_body_T_parent.transpose())
            // << std::endl;

            // std::cout << "[debug] root lin vel in root frame = " <<
            // btBulletUtil::btVectorTotVector0(body.m_parent_vel_rel).transpose()
            // << std::endl;
        }
        if (type >= POSITION_VELOCITY_ACCELERATION)
        {
            ...
            // body.m_parent_acc_rel = body.m_body_T_parent.transpose() *
            // body.m_parent_acc_rel;

            // 20200107�� inputs are accels in world frame, convert them to
            // local frame
            body.m_body_ang_acc_rel(0) = dot_u(body.m_q_index + 0);
            body.m_body_ang_acc_rel(1) = dot_u(body.m_q_index + 1);
            body.m_body_ang_acc_rel(2) = dot_u(body.m_q_index + 2);
            // std::cout << "[debug] root ang accel in world frame = " <<
            // btBulletUtil::btIDVectorTotVector0(body.m_body_ang_acc_rel).transpose()
            // << std::endl;
            body.m_body_ang_acc_rel =
                body.m_body_T_parent * body.m_body_ang_acc_rel;
            // std::cout << "[debug] root ang accel in body frame = " <<
            // btBulletUtil::btIDVectorTotVector0(body.m_body_ang_acc_rel).transpose()
            // << std::endl;
            ...
            body.m_parent_acc_rel(1) = dot_u(body.m_q_index + 4);
            body.m_parent_acc_rel(2) = dot_u(body.m_q_index + 5);
            // std::cout << "[debug] root lin accel world = " <<
            // btBulletUtil::btIDVectorTotVector0(body.m_parent_acc_rel).transpose()
            // << std::endl;
            // body.m_parent_acc_rel = body.m_body_T_parent *
            // body.m_parent_acc_rel;
            // std::cout << "[debug] root lin accel body = " <<
            // btBulletUtil::btIDVectorTotVector0(body.m_parent_acc_rel).transpose()
            // << std::endl;
        }
    }
    ...
    for (idArrayIdx i = 0; i < m_body_spherical_list.size(); i++)
    {
        ...
        T = (transformX(q(body.m_q_index)) * transformY(q(body.m_q_index + 1)) *
             transformZ(q(body.m_q_index + 2)))
                .transpose();
        // std::cout << "[debug] spherical joint eulerangle = " <<
        // q(body.m_q_index) << " " << q(body.m_q_index + 1) << " " <<
        // q(body.m_q_index + 2) << std::endl; std::cout << "[debug] spherical
        // rot mat(parent to local) = \n" <<
        // btBulletUtil::btIDMatrixTotMatrix(T) << std::endl;
        body.m_body_T_parent = T * body.m_body_T_parent_ref;
        // std::cout << "[debug] spherical parent to body rot = \n" <<
        // btBulletUtil::btIDMatrixTotMatrix(body.m_body_T_parent) << std::endl;

        // std::cout << "[warn] body.m_parent_pos_parent_body =
        // vec3(btVector3(0.0, -0.37, 0.0));\n" << std::endl;
        ...
        // body.m_parent_pos_parent_body = body.m_body_T_parent *
        // body.m_parent_pos_parent_body; body.m_parent_pos_parent_body =
        // body.m_body_T_parent * body.m_parent_pos_parent_body;
        ...
        if (type >= POSITION_VELOCITY)
        {
...

            {
                ...
                // btBulletUtil::btIDVectorTotVector0(body.m_body_ang_vel_rel).transpose()
                // << std::endl; body.m_body_ang_vel_rel = body.m_body_T_parent
                // * body.m_body_ang_vel_rel; convert ang vel relative from
                // parent frame to body-fixed frame std::cout << "[debug] ang
                // vel rel in body fixed frame = \n" <<
                // btBulletUtil::btIDVectorTotVector0(body.m_body_ang_vel_rel).transpose()
                // << std::endl;

                // linear vel: should be 0
                // body.m_parent_vel_rel = body.m_body_T_parent *
                // body.m_parent_vel_rel;
            }
        ...
        if (type >= POSITION_VELOCITY_ACCELERATION)
        {
            ...
            // body.m_body_ang_acc_rel = body.m_body_T_parent *
            // body.m_body_ang_acc_rel;
            ...
            body.m_parent_acc_rel =
                body.m_body_T_parent * body.m_parent_acc_rel;
    ...
    // calculations for root body
    {
        ...
        ///btBulletUtil::btIDMatrixTotMatrix(body.m_body_T_world) << std::endl;
        ...
        if (type >= POSITION_VELOCITY_ACCELERATION)
        {
            ...
            // NOTE: assumption: dot(J_JR) = 0; true here, but not for general
            // joints
            ...
            // NOTE: To get correct acceleration kinematics, just set
            // world_gravity to zero
            body.m_body_acc =
                body.m_body_acc - body.m_body_T_parent * m_world_gravity;
        }

        // std::cout << "[debug] root joint body pos = " <<
        // btBulletUtil::btIDVectorTotVector1(body.m_body_pos).transpose() <<
        // std::endl; std::cout << "[debug] root to world rot mat = " <<
        // btBulletUtil::btIDMatrixTotMatrix(body.m_body_T_world) << std::endl;
        // std::cout << "[debug] root joint ang vel in fixed-body frame = " <<
        // btBulletUtil::btIDVectorTotVector1(body.m_body_ang_vel).transpose()
        // << std::endl; std::cout << "[debug] root joint lin vel in fixed-body
        // frame = " <<
        // btBulletUtil::btIDVectorTotVector1(body.m_body_vel).transpose() <<
        // std::endl; std::cout << "[debug] root joint ang accel vel in
        // fixed-body frame = " <<
        // btBulletUtil::btIDVectorTotVector1(body.m_body_ang_acc).transpose()
        // << std::endl; std::cout << "[debug] root joint lin accel vel in
        // fixed-body frame = " <<
        // btBulletUtil::btIDVectorTotVector1(body.m_body_acc).transpose() <<
        // std::endl;
    }
    ...
    for (idArrayIdx i = 1; i < m_body_list.size(); i++)
    {
        ...
        // will be required if we add force elements (eg springs between bodies,
        // or contacts)  not required right now added here for debugging
        // purposes
        body.m_body_pos = body.m_body_T_parent *
                          (parent.m_body_pos + body.m_parent_pos_parent_body);
        body.m_body_T_world = body.m_body_T_parent * parent.m_body_T_world;
        // std::cout << "[debug] link spherical " << i << " body pos in body
        // fixed frame = " <<
        // btBulletUtil::btIDVectorTotVector1(body.m_body_pos).transpose() <<
        // std::endl; std::cout << "[debug] link spherical " << i << " world to
        // body = \n" << btBulletUtil::btIDMatrixTotMatrix(body.m_body_T_world)
        // << std::endl; std::cout << "[ID debug] revolute world to body = \n"
        // << btBulletUtil::btIDMatrixTotMatrix(body.m_body_T_world) <<
        // std::endl;
        ...
        if (type >= POSITION_VELOCITY)
        {
            ...
            body.m_body_ang_vel = body.m_body_T_parent * parent.m_body_ang_vel +
                                  body.m_body_ang_vel_rel;
            //// std::cout << "[debug] body ang vel in body fixed frame = " <<
            ///btBulletUtil::btIDVectorTotVector0(body.m_body_ang_vel).transpose()
            ///<< std::endl;
            body.m_body_vel =
                body.m_body_T_parent *
                (parent.m_body_vel +
                 ...
                 body.m_parent_vel_rel);
            //// std::cout << "[debug] body vel in body fixed frame = " <<
            ///btBulletUtil::btIDVectorTotVector0(body.m_body_vel).transpose()
            ///<< std::endl;
        }
        if (type >= POSITION_VELOCITY_ACCELERATION)
        {
            ...
            // NOTE: assumption: dot(J_JR) = 0; true here, but not for general
            // joints �빫ʽ�Ǻ� m_body_ang_vel_rel: body frame m_body_ang_acc_rel:
            // body frame
            body.m_body_ang_acc =
                body.m_body_T_parent * parent.m_body_ang_acc -
                body.m_body_ang_vel_rel.cross(body.m_body_T_parent *
                                              parent.m_body_ang_vel) +
                body.m_body_ang_acc_rel;
            //// std::cout << "[debug] body ang accel in body fixed frame = " <<
            ///btBulletUtil::btIDVectorTotVector0(body.m_body_ang_acc).transpose()
            ///<< std::endl;

            // ����body
            // acc��Ҳ����body�߼��ٶȵ�ʱ�򣬺ܶ�ֵʹ�õĶ���parent����ϵ�µġ���body��ص���:
            /*
                    parent ����ϵ��: parent.m_body_acc, parent.m_body_ang_acc,
               parent.m_body_ang_vel,parent.m_body_ang_vel,
               parent.m_body_ang_vel ������ϵ��:body.m_parent_pos_parent_body,
               body.m_parent_vel_rel, body.m_parent_acc_rel

            */
            //// std::cout << "[debug] check m_parent_pos_parent_body = " <<
            ///btBulletUtil::btIDVectorTotVector0(body.m_parent_pos_parent_body).transpose()
            ///<< std::endl;
            ...
            body.m_body_acc =
                body.m_body_T_parent *
                (parent.m_body_acc +
                 parent.m_body_ang_acc.cross(body.m_parent_pos_parent_body) +
                 parent.m_body_ang_vel.cross(parent.m_body_ang_vel.cross(
                     body.m_parent_pos_parent_body)) +
...
void MultiBodyTree::MultiBodyImpl::addRelativeJacobianComponent(RigidBody &body)
{
    ...
    case FLOATING:
        ...
        setMat3xElem(0, idx + 3, body.m_body_T_parent(0, 0),
                     &body.m_body_Jac_T);
        setMat3xElem(0, idx + 4, body.m_body_T_parent(1, 0),
                     &body.m_body_Jac_T);
        setMat3xElem(0, idx + 5, body.m_body_T_parent(2, 0),
                     &body.m_body_Jac_T);

        setMat3xElem(1, idx + 3, body.m_body_T_parent(0, 1),
                     &body.m_body_Jac_T);
        setMat3xElem(1, idx + 4, body.m_body_T_parent(1, 1),
                     &body.m_body_Jac_T);
        setMat3xElem(1, idx + 5, body.m_body_T_parent(2, 1),
                     &body.m_body_Jac_T);

        setMat3xElem(2, idx + 3, body.m_body_T_parent(0, 2),
                     &body.m_body_Jac_T);
        setMat3xElem(2, idx + 4, body.m_body_T_parent(1, 2),
                     &body.m_body_Jac_T);
        setMat3xElem(2, idx + 5, body.m_body_T_parent(2, 2),
                     &body.m_body_Jac_T);
...
int MultiBodyTree::MultiBodyImpl::calculateJacobians(const vecx &q,
                                                     const vecx &u,
                                                     const KinUpdateType type)
{
    if (q.size() != m_num_dofs || u.size() != m_num_dofs)
    ...
    for (idArrayIdx i = 1; i < m_body_list.size(); i++)
    {
        ...
        mul(tildeOperator(body.m_parent_pos_parent_body), parent.m_body_Jac_R,
            &m_m3x);
        ...
        if (type >= POSITION_VELOCITY)
        {
            body.m_body_dot_Jac_R_u =
                body.m_body_T_parent * parent.m_body_dot_Jac_R_u -
                body.m_body_ang_vel_rel.cross(body.m_body_T_parent *
                                              parent.m_body_ang_vel);
            body.m_body_dot_Jac_T_u =
                body.m_body_T_parent *
                (parent.m_body_dot_Jac_T_u +
                 parent.m_body_dot_Jac_R_u.cross(
                     body.m_parent_pos_parent_body) +
                 parent.m_body_ang_vel.cross(parent.m_body_ang_vel.cross(
                     body.m_parent_pos_parent_body)) +
                 2.0 * parent.m_body_ang_vel.cross(body.m_parent_vel_rel));
...
static inline void setThreeDoFJacobians(const int dof, vec3 &Jac_JR,
                                        vec3 &Jac_JT)
{
    switch (dof)
    {
...
int MultiBodyTree::MultiBodyImpl::calculateMassMatrix(
    const vecx &q, const bool update_kinematics, const bool initialize_matrix,
    const bool set_lower_triangular_matrix, matxx *mass_matrix)
{
    ...
    // in "Efficient Dynamic Simulation of Robotic Mechanisms" (Walker and Orin,
    // 1982) (Later named "Composite Rigid Body Algorithm" by Featherstone).
    //
    // This implementation, however, handles branched systems and uses a
    // formulation centered on the origin of the body-fixed frame to avoid
    // re-computing various quantities at the com.
        ...
        mass_matrix->cols() != m_num_dofs)
    {
        bt_id_error_message("Dimension error. System has %d DOFs,\n"
                            ...
                            m_num_dofs, static_cast<int>(q.size()),
                            static_cast<int>(mass_matrix->rows()),
    ...
    if (update_kinematics)
    {
        ...
        {
            RigidBody &body = m_body_list[m_body_revolute_list[i]];
            // from reference orientation (q=0) of body-fixed frame to current
            // orientation
            mat33 body_T_body_ref;
            bodyTParentFromAxisAngle(body.m_Jac_JR, q(body.m_q_index),
                                     &body_T_body_ref);
    ...
    for (int i = m_body_list.size() - 1; i >= 0; i--)
    {
        ...
        for (idArrayIdx c = 0; c < m_child_indices[i].size(); c++)
        {
            ...
            body.m_body_subtree_mass_com +=
                body_T_child * child.m_body_subtree_mass_com +
                child.m_parent_pos_parent_body * child.m_subtree_mass;
            body.m_body_subtree_I_body += body_T_child *
                                          child.m_body_subtree_I_body *
                                          child.m_body_T_parent;

            if (child.m_subtree_mass > 0)
            {
                // Shift the reference point for the child subtree inertia using
                // the Huygens-Steiner ("parallel axis") theorem. (First shift
                // from child origin to child com, then from there to this
                // body's origin)
                vec3 r_com = body_T_child * child.m_body_subtree_mass_com /
                             child.m_subtree_mass;
                mat33 tilde_r_child_com = tildeOperator(r_com);
                mat33 tilde_r_body_com =
                    tildeOperator(child.m_parent_pos_parent_body + r_com);
                body.m_body_subtree_I_body +=
                    child.m_subtree_mass *
                    (tilde_r_child_com * tilde_r_child_com -
                     tilde_r_body_com * tilde_r_body_com);
            }
        }
    }
    ...
    for (int i = m_body_list.size() - 1; i >= 0; i--)
    {
        ...
        const int q_index_max =
            q_index_min + jointNumDoFs(body.m_joint_type) - 1;
        ...
        for (int col = q_index_max; col >= q_index_min; col--)
        {
            ...
            vec3 body_eom_rot = body.m_body_subtree_I_body * Jac_JR +
                                body.m_body_subtree_mass_com.cross(Jac_JT);
            vec3 body_eom_trans = body.m_subtree_mass * Jac_JT -
                                  body.m_body_subtree_mass_com.cross(Jac_JR);
            setMatxxElem(col, col,
                         Jac_JR.dot(body_eom_rot) + Jac_JT.dot(body_eom_trans),
                         mass_matrix);
            ...
            // rest of the mass matrix column upwards
            {
                ...
                for (int row = col - 1; row >= q_index_min; row--)
                {
                    if (SPHERICAL == body.m_joint_type)
                    {
                        ...
                        const double Mrc = Jac_JR.dot(body_eom_rot) +
                                           Jac_JT.dot(body_eom_trans);
                ...
                while (parent_idx >= 0)
                {
                    ...
                    const mat33 parent_T_child =
                        child_body.m_body_T_parent.transpose();
                    body_eom_rot = parent_T_child * body_eom_rot;
                    body_eom_trans = parent_T_child * body_eom_trans;
                    body_eom_rot += child_body.m_parent_pos_parent_body.cross(
                        body_eom_trans);

                    const int parent_body_q_index_min = parent_body.m_q_index;
                    const int parent_body_q_index_max =
                        parent_body_q_index_min +
                        jointNumDoFs(parent_body.m_joint_type) - 1;
                    vec3 Jac_JR = parent_body.m_Jac_JR;
                    vec3 Jac_JT = parent_body.m_Jac_JT;
                    for (int row = parent_body_q_index_max;
                         row >= parent_body_q_index_min; row--)
                    {
                        if (SPHERICAL == parent_body.m_joint_type)
                        {
                            // todo: review
                            setThreeDoFJacobians(row - parent_body_q_index_min,
                                                 Jac_JR, Jac_JT);
                        }
                        // set jacobians for 6-DoF joints
                        if (FLOATING == parent_body.m_joint_type)
                        {
                            setSixDoFJacobians(row - parent_body_q_index_min,
                                               Jac_JR, Jac_JT);
                        }
                        const double Mrc = Jac_JR.dot(body_eom_rot) +
                                           Jac_JT.dot(body_eom_trans);
...
int MultiBodyTree::MultiBodyImpl::getUserInt(const int body_index,
                                             int *user_int) const
{
    CHECK_IF_BODY_INDEX_IS_VALID(body_index);
...
int MultiBodyTree::MultiBodyImpl::getUserPtr(const int body_index,
                                             void **user_ptr) const
{
    CHECK_IF_BODY_INDEX_IS_VALID(body_index);
...
int MultiBodyTree::MultiBodyImpl::setUserInt(const int body_index,
                                             const int user_int)
{
    CHECK_IF_BODY_INDEX_IS_VALID(body_index);
...
int MultiBodyTree::MultiBodyImpl::setUserPtr(const int body_index,
                                             void *const user_ptr)
{
    CHECK_IF_BODY_INDEX_IS_VALID(body_index);
...
int MultiBodyTree::MultiBodyImpl::getBodyOrigin(int body_index,
...
int MultiBodyTree::MultiBodyImpl::getBodyCoM(int body_index,
                                             vec3 *world_com) const
{
    CHECK_IF_BODY_INDEX_IS_VALID(body_index);
    const RigidBody &body = m_body_list[body_index];
    if (body.m_mass > 0)
...
int MultiBodyTree::MultiBodyImpl::getBodyTransform(int body_index,
                                                   mat33 *world_T_body) const
{
    CHECK_IF_BODY_INDEX_IS_VALID(body_index);
    const RigidBody &body = m_body_list[body_index];
...
int MultiBodyTree::MultiBodyImpl::getBodyAngularVelocity(
    int body_index, vec3 *world_omega) const
{
    CHECK_IF_BODY_INDEX_IS_VALID(body_index);
    const RigidBody &body = m_body_list[body_index];
...
int MultiBodyTree::MultiBodyImpl::getBodyLinearVelocity(
    int body_index, vec3 *world_velocity) const
{
    CHECK_IF_BODY_INDEX_IS_VALID(body_index);
    const RigidBody &body = m_body_list[body_index];
...
int MultiBodyTree::MultiBodyImpl::getBodyLinearVelocityCoM(
    int body_index, vec3 *world_velocity) const
{
    CHECK_IF_BODY_INDEX_IS_VALID(body_index);
    const RigidBody &body = m_body_list[body_index];
    ...
    *world_velocity = body.m_body_T_world.transpose() *
                      (body.m_body_vel + body.m_body_ang_vel.cross(com));
    return 0;
}

int MultiBodyTree::MultiBodyImpl::getBodyAngularAcceleration(
    int body_index, vec3 *world_dot_omega) const
{
    CHECK_IF_BODY_INDEX_IS_VALID(body_index);
    const RigidBody &body = m_body_list[body_index];
...
int MultiBodyTree::MultiBodyImpl::getBodyLinearAcceleration(
    int body_index, vec3 *world_acceleration) const
{
    CHECK_IF_BODY_INDEX_IS_VALID(body_index);
    const RigidBody &body = m_body_list[body_index];
...
int MultiBodyTree::MultiBodyImpl::getJointType(const int body_index,
                                               JointType *joint_type) const
{
    CHECK_IF_BODY_INDEX_IS_VALID(body_index);
...
int MultiBodyTree::MultiBodyImpl::getParentRParentBodyRef(const int body_index,
                                                          vec3 *r) const
{
    CHECK_IF_BODY_INDEX_IS_VALID(body_index);
...
int MultiBodyTree::MultiBodyImpl::getBodyTParentRef(const int body_index,
                                                    mat33 *T) const
{
    CHECK_IF_BODY_INDEX_IS_VALID(body_index);
...
int MultiBodyTree::MultiBodyImpl::getBodyAxisOfMotion(const int body_index,
                                                      vec3 *axis) const
{
    CHECK_IF_BODY_INDEX_IS_VALID(body_index);
...
int MultiBodyTree::MultiBodyImpl::getDoFOffset(const int body_index,
                                               int *q_index) const
{
    CHECK_IF_BODY_INDEX_IS_VALID(body_index);
...
int MultiBodyTree::MultiBodyImpl::setBodyMass(const int body_index,
                                              const idScalar mass)
{
    CHECK_IF_BODY_INDEX_IS_VALID(body_index);
...
int MultiBodyTree::MultiBodyImpl::setBodyFirstMassMoment(
    const int body_index, const vec3 &first_mass_moment)
{
    CHECK_IF_BODY_INDEX_IS_VALID(body_index);
    m_body_list[body_index].m_body_mass_com = first_mass_moment;
    return 0;
}
int MultiBodyTree::MultiBodyImpl::setBodySecondMassMoment(
    const int body_index, const mat33 &second_mass_moment)
{
    CHECK_IF_BODY_INDEX_IS_VALID(body_index);
    m_body_list[body_index].m_body_I_body = second_mass_moment;
    return 0;
}
int MultiBodyTree::MultiBodyImpl::getBodyMass(const int body_index,
                                              idScalar *mass) const
{
    CHECK_IF_BODY_INDEX_IS_VALID(body_index);
...
int MultiBodyTree::MultiBodyImpl::getBodyFirstMassMoment(
    const int body_index, vec3 *first_mass_moment) const
{
    CHECK_IF_BODY_INDEX_IS_VALID(body_index);
    *first_mass_moment = m_body_list[body_index].m_body_mass_com;
    return 0;
}
int MultiBodyTree::MultiBodyImpl::getBodySecondMassMoment(
    const int body_index, mat33 *second_mass_moment) const
{
    CHECK_IF_BODY_INDEX_IS_VALID(body_index);
...
int MultiBodyTree::MultiBodyImpl::addUserForce(const int body_index,
...
int MultiBodyTree::MultiBodyImpl::addUserMoment(const int body_index,
                                                const vec3 &body_moment)
{
    CHECK_IF_BODY_INDEX_IS_VALID(body_index);
...
int MultiBodyTree::MultiBodyImpl::getUserForce(const int body_index,
                                               vec3 &force) const
{
    CHECK_IF_BODY_INDEX_IS_VALID(body_index);
...
int MultiBodyTree::MultiBodyImpl::getUserMoment(const int body_index,
                                                vec3 &moment) const
{
    CHECK_IF_BODY_INDEX_IS_VALID(body_index);
...
int MultiBodyTree::MultiBodyImpl::getBodyDotJacobianTransU(
    const int body_index, vec3 *world_dot_jac_trans_u) const
{
    CHECK_IF_BODY_INDEX_IS_VALID(body_index);
    const RigidBody &body = m_body_list[body_index];
    *world_dot_jac_trans_u =
        body.m_body_T_world.transpose() * body.m_body_dot_Jac_T_u;
    return 0;
}

int MultiBodyTree::MultiBodyImpl::getBodyDotJacobianRotU(
    const int body_index, vec3 *world_dot_jac_rot_u) const
{
    CHECK_IF_BODY_INDEX_IS_VALID(body_index);
    const RigidBody &body = m_body_list[body_index];
    *world_dot_jac_rot_u =
        body.m_body_T_world.transpose() * body.m_body_dot_Jac_R_u;
    return 0;
}

int MultiBodyTree::MultiBodyImpl::getBodyJacobianTrans(
    const int body_index, mat3x *world_jac_trans) const
{
    CHECK_IF_BODY_INDEX_IS_VALID(body_index);
    const RigidBody &body = m_body_list[body_index];
...
int MultiBodyTree::MultiBodyImpl::getBodyJacobianRot(const int body_index,
                                                     mat3x *world_jac_rot) const
{
    CHECK_IF_BODY_INDEX_IS_VALID(body_index);
    const RigidBody &body = m_body_list[body_index];
