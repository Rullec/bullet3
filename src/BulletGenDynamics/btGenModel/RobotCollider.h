...
// this class is typically used to avoid the collision between parent link and
// its child.
struct btGenRobotCollider : public btGenCollisionObject
{
public:
    btGenRobotCollider(cRobotModelDynamics *model, int link_id,
    ...
    virtual bool
    checkCollideWithOverride(const btCollisionObject *co) const override;

    virtual void ApplyForce(const tVector &force,
                            const tVector &pos) override final;
    ...
    virtual void PushState(const std::string &tag,
                           bool only_vel_and_force = false) override final;
