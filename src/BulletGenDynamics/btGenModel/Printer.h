...
public:
    ...
    static void FormatScreenLog(const char *fmt, ...)
    {
        ...
        const std::string &log =
            std::string(buf, std::min(real_size, static_cast<int>(buf_size)));
    ...
    static void Log(const std::vector<double> &data, int group_num,
                    const char *prefix, const char *file_path)
    {
        std::fstream fout;
        fout.precision(20);
        fout.open(file_path, std::ios::out);
    ...
    static void Log(const tVectorXd &data, int group_num, const char *prefix,
    ...
    static void LogCSV(const tVectorXd &data, int group_num, const char *prefix,
                       const char *file_path)
    {
        std::fstream fout;
        fout.open(file_path, std::ios::out);
    ...
    static void LogCSV(const std::vector<double> &data, int group_num,
                       const char *prefix, const char *file_path)
    {
        std::fstream fout;
        fout.open(file_path, std::ios::out);
    ...
    // static void Log(std::vector<double>& data, int group_num, const char*
    // prefix, const char* file_path) { 	std::fstream fout; 	fout.open(file_path,
    //std::ios::out); 	const int n = static_cast<int>(data.size()) / group_num;
    //	for (int i = 0; i < n; i++) {
    //		fout << i << std::endl;
    //		for (int j = 0; j < group_num; j++) {
    //			fout << j << ", " << data[i * group_num + j] <<
    ...
    //	fout.close();
    //}
    static void Log(const std::vector<std::vector<double>> &data, int group_num,
                    const char *prefix, const char *file_path)
    {
        if (data.empty())
            return;
    ...
    static void Log(const EIGEN_V_tVector3f &data, const char *prefix,
                    const char *file_path)
    {
        std::fstream fout;
        fout.open(file_path, std::ios::out);
        for (unsigned int i = 0; i < data.size(); i++)
        {
            fout << i << ": " << data[i][0] << ", " << data[i][1] << ", "
                 << data[i][2] << std::endl;
        }
        fout.close();
    }

    static void Log(const EIGEN_V_tVector3d &data, const char *prefix,
                    const char *file_path)
    {
        std::fstream fout;
        fout.open(file_path, std::ios::out);
    ...
    static void Log(const EIGEN_V_tVector3d &data, int group_num,
                    const char *prefix, const char *file_path)
    {
        std::fstream fout;
        const int n = static_cast<int>(data.size()) / group_num;
        ...
        for (int i = 0; i < n; i++)
        {
            fout << i << ": \n";
            for (int j = 0; j < group_num; ++j)
                fout << data[st + j][0] << ", " << data[st + j][1] << ", "
                     << data[st + j][2] << std::endl;
            st += group_num;
        }
        fout.close();
    }
    ...
    static void Log(const EIGEN_V_tVector3d &data, int group_num,
                    const char *prefix, const char *file_path, bool norm)
    {
        std::fstream fout;
        const int n = static_cast<int>(data.size()) / group_num;
    ...
    static void Log(const EIGEN_V_tVector &data, int group_num,
                    const char *prefix, const char *file_path)
    {
        std::fstream fout;
        fout.precision(8);
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            for (int j = 0; j < group_num; ++j)
                fout << data[st + j][0] << ", " << data[st + j][1] << ", "
                     << data[st + j][2] << ", " << data[st + j][3] << std::endl;
            st += group_num;
        }
        fout.close();
    }
    ...
    static void Log(const tMatrixXd &m, std::vector<int> &col_id,
                    const char *file_path)
    {
        std::fstream fout;
        fout.open(file_path, std::ios::out);
    ...
    static void Log(const EIGEN_VV_MATXD &ms, std::vector<std::string> names,
                    const char *file_path)
    {
        std::fstream fout;
        fout.open(file_path, std::ios::out);
    ...
    static void Log(const std::vector<const std::vector<double> *> &data,
                    int group_num, const char *prefix, const char *file_path)
    {
        std::fstream fout;
        fout.open(file_path, std::ios::out);
        fout << prefix << '\n';
    ...
    static void Log(const EIGEN_VV_tVector3d &data,
                    std::vector<std::string> names, const char *file_path)
    {
        std::fstream fout;
        fout.open(file_path, std::ios::out);
    ...
    static void Log(const EIGEN_V_VECXD &data, const char *prefix,
                    const char *file_path)
    {
        std::fstream fout;
        fout.precision(10);
