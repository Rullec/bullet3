...
public:
    ...
    virtual void PushState(const std::string &tag,
                           bool only_vel_force_record = false) override;
    virtual void PopState(const std::string &tag,
                          bool only_vel_force_record = false) override;

    // void TestAbsCartesianConvertMat(const tMatrixXd & mat, const tVectorXd &
    // vec);
...
protected:
    ...
    // void EvalFunc(double dt, const tVector& AngVelNew, const tVector&
    // AngVelOld, tVector & func_value);
    ...
    void EvalFuncDeriNumerically(double dt, const tVector &AngVelNew,
                                 tMatrix &mat);
