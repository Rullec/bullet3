#ifndef UNREVISED_LEMKE_SOLVER
#define UNREVISED_LEMKE_SOLVER
#pragma once
#include <algorithm>
#include <fstream>
#include <limits>
...
public:
    ...
    static double ComputeZeroTolerance(const tMatrixXd &M)
    {
    ...
    static bool IsSolution(const tMatrixXd &M, const tVectorXd &q,
                           const tVectorXd &z, double zero_tol = -1);
    ...
    bool SolveLcpLemke(const tMatrixXd &M, const tVectorXd &q, tVectorXd *z,
                       int *num_pivots,
                       const double &zero_tol = double(-1)) const;
...
private:
    ...
    static void SelectSubMatrixWithCovering(const tMatrixXd &in,
                                            const std::vector<int> &rows,
                                            const std::vector<int> &cols,
                                            tMatrixXd *out);
    static void SelectSubColumnWithCovering(const tMatrixXd &in,
                                            const std::vector<int> &rows,
                                            int column, tVectorXd *out);
    static void SelectSubVector(const tVectorXd &in,
                                const std::vector<int> &rows, tVectorXd *out);
    static void SetSubVector(const tVectorXd &v_sub,
                             const std::vector<int> &indices, tVectorXd *v);
    ...
    bool LemkePivot(const tMatrixXd &M, const tVectorXd &q, int driving_index,
                    double zero_tol, tVectorXd *M_bar_col,
                    tVectorXd *q_bar) const;
    bool ConstructLemkeSolution(const tMatrixXd &M, const tVectorXd &q,
                                int artificial_index, double zero_tol,
                                tVectorXd *z) const;
    int FindComplementIndex(const LCPVariable &query) const;
    void DetermineIndexSets() const;
    bool FindBlockingIndex(const double &zero_tol, const tVectorXd &matrix_col,
                           const tVectorXd &ratios, int *blocking_index) const;
    ...
    mutable tMatrixXd M_alpha_beta_, M_alpha_bar_beta_;
    mutable tVectorXd q_alpha_, q_alpha_bar_, q_prime_beta_prime_,
