...
template <int a, int b> struct EigenSizeMinPreferDynamic
{
    // clang-format off
...
template <int a, int b> struct EigenSizeMinPreferFixed
{
    // clang-format off
  static constexpr int value = (a == 0 || b == 0) ? 0 :
                               (a == 1 || b == 1) ? 1 :
...
template <int a, int b> struct MultiplyEigenSizes
{
...
/// documentation</a>.
template <typename PlainObjectType> class EigenPtr
{
public:
