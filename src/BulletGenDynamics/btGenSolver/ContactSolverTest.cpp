...
void btGenContactSolver::TestCartesianForceToCartesianVel()
{
	...
	for (int i = 0; i < n_group; i++)
	{
		...
		contact_point_cartesian_vel[i] = mColGroupData[i]->mConvertCartesianForceToVelocityMat * x_lcp + mColGroupData[i]->mConvertCartesianForceToVelocityVec;
		// std::cout << "for group " << i << " "
		// 		  << "c2v mat = \n"
		// 		  << mColGroupData[i]->mConvertCartesianForceToVelocityMat << std::endl;
		// std::cout << "for group " << i << " "
		// 		  << "c2v vec = \n"
		// 		  << mColGroupData[i]->mConvertCartesianForceToVelocityVec.transpose() << std::endl;
	}
	...
	// std::cout << "Q = " << mMultibodyArray[0]->GetGeneralizedForce().transpose() << std::endl;
	...
	for (int i = 0; i < mNumContactPoints; i++)
	{
		...
		{
			err = true;
			std::cout << "[error] convert cartesian abs vel: for contact " << i << "/" << mNumContactPoints << "---------------\n";
			...
			std::cout << "body0 diff = " << (body0_pred_vel - body0_true_vel).transpose() << std::endl;
			std::cout << "body 1 pred vel = " << body1_pred_vel.transpose() << std::endl;
			std::cout << "body 1 true vel = " << body1_true_vel.transpose() << std::endl;
			std::cout << "body 1 diff = " << (body1_pred_vel - body1_true_vel).transpose() << std::endl;
			...
			if (body0_diff > EPS)
			{
					...
					true == UpcastRobotCollider(data->mBodyA)->mModel->IsGeneralizedMaxVel())
				{
					...
					std::cout << "[veirfy abs vel] qdot = " << UpcastRobotCollider(data->mBodyA)->mModel->Getqdot().transpose() << std::endl;
				}
			}
			if (body1_diff > EPS)
			{
					...
					true == UpcastRobotCollider(data->mBodyB)->mModel->IsGeneralizedMaxVel())
				{
					...
					std::cout << "[veirfy abs vel] qdot = " << UpcastRobotCollider(data->mBodyB)->mModel->Getqdot().transpose() << std::endl;
				}
			}
		}
	}
	...
	for (int i = 0; i < mNumJointLimitConstraints; i++)
	{
		...
		// std::cout << "multibody pred qdot = " << (contact_point_cartesian_vel[group_id]).transpose() << std::endl;
		double qdot_true = data->multibody->Getqdot()[data->dof_id];
		double qdot_pred = contact_point_cartesian_vel[group_id][mNumContactPoints * 3 + i];
		if (data->is_upper_bound) qdot_pred *= -1;
		double diff = std::fabs(qdot_true - qdot_pred);
		// std::cout << "constraint " << i << " dof " << data->dof_id << " pred vel = " << qdot_pred << ", true vel = " << qdot_true << std::endl;
		if (diff > EPS)
		{
			std::cout << "[error] convert cartesian abs vel: for joint limit constraint " << i << ", dof = " << data->dof_id << std::endl;
			...
			if (true == data->multibody->IsGeneralizedMaxVel())
			{
				...
				std::cout << "[veirfy q] qdot = " << data->multibody->Getqdot().transpose() << std::endl;
			}
		}
	}
	if (err)
...
void btGenContactSolver::TestCartesianForceToCartesianRelVel(const tMatrixXd& convert_mat, const tVectorXd& convert_vec)
{
	...
	for (int i = 0; i < mNumContactPoints; i++)
	{
		...
		if (diff > EPS)
		{
			...
			std::cout << "total contact num = " << mNumContactPoints << std::endl;
	...
	for (int i = 0; i < mNumJointLimitConstraints; i++)
	{
		...
		{
			err = true;
			std::cout << "[error] convert cartesian force to rel vel: for joint limit constraint " << i << ", dof = " << data->dof_id << std::endl;
			std::cout << "pred vel " << qdot_pred << std::endl;
			std::cout << "true vel " << qdot_true << std::endl;
			std::cout << "diff = " << diff << std::endl;
...
void btGenContactSolver::TestCartesianForceToNormalAndTangetRelVel(const tMatrixXd& normal_mat, const tVectorXd& normal_vec, const tMatrixXd& tan_mat, const tVectorXd& tan_vec)
{
	...
	tVectorXd rel_vel_tangent_pred = tVectorXd::Zero(rel_vel_normal_pred.size());
	...
	for (int i = 0; i < mNumContactPoints; i++)
	{
		...
		if (mEnableFrictionalLCP)
		{
			...
			double diff = (true_tan_relvel - pred_tan_relvel).cwiseAbs().maxCoeff();
			if (diff > EPS)
			{
				err = true;
				std::cout << "[error] cartesian force to tangent rel vel for contact " << i << "---------------\n";
		...
		double diff = (pred_normal_relvel - pred_normal_relvel).cwiseAbs().maxCoeff();
		if (diff > EPS)
		{
			err = true;
			std::cout << "[error] cartesian force to normal rel vel for contact " << i << "---------------\n";
	...
	for (int i = 0; i < mNumJointLimitConstraints; i++)
	{
		...
		if (data->is_upper_bound) qdot_pred *= -1;
		// std::cout << "qdot pred = " << rel_vel_normal_pred.transpose() << std::endl;
		// std::cout << "qdot = " << data->multibody->Getqdot().transpose() << std::endl;
		double diff = qdot_true - qdot_pred;
		if (diff > EPS)
		{
			err = true;
			std::cout << "[error] cartesian force to joint limit qdot for contact " << i << "---------------\n";
...
void btGenContactSolver::TestCartesianForceToNormalAndTangetResultBasedRelVel(const tMatrixXd& normal_mat, const tVectorXd& normal_vec, const tMatrixXd& tan_mat, const tVectorXd& tan_vec)
{
	...
	int final_shape = single_size * mNumContactPoints + mNumJointLimitConstraints;
	...
	// get normal and tangent vel prediction on each point
	tVectorXd rel_vel_normal_pred = normal_mat * x_lcp + normal_vec;
	tVectorXd rel_vel_tangent_pred = tVectorXd::Zero(mNumFrictionDirs * mNumContactPoints);
	if (mEnableFrictionalLCP)
	{
		// std::cout <<"[rel vel tan] vec = " << tan_vec.transpose() << std::endl;
		rel_vel_tangent_pred = tan_mat * x_lcp + tan_vec;
	}
	...
	for (int i = 0; i < mNumContactPoints; i++)
	{
		...
		if (mEnableFrictionalLCP)
		{
			applied_force.segment(3 * i, 3) = data->mS * x_lcp.segment(single_size * i, single_size);
		}
		else
		{
	...
	for (int i = 0; i < mNumContactPoints; i++)
	{
		...
		if (mEnableFrictionalLCP)
		{
			true_tan_relvel = data->mD.transpose() * (true_rel_vel - data->mNormalPointToA * true_normal_relvel[0]).segment(0, 3);
		}

		// std::cout <<"true 1 = " << true_tan_relvel << std::endl;
		tVectorXd pred_normal_relvel = rel_vel_normal_pred.segment(i, 1);
		tVectorXd pred_tan_relvel = rel_vel_tangent_pred.segment(i * mNumFrictionDirs, mNumFrictionDirs);
		double diff = (pred_normal_relvel - true_normal_relvel).cwiseAbs().maxCoeff();
		if (
			diff > EPS)
		{
			std::cout << "[error] convert x into decomposed normal vel, for contact " << i << "---------------\n";
		...
		if (mEnableFrictionalLCP && (pred_tan_relvel - true_tan_relvel).cwiseAbs().maxCoeff() > EPS)
		{
			std::cout << "[error] convert x into decomposed tangent vel, for contact " << i << "---------------\n";
			std::cout << "total contact = " << mNumContactPoints << std::endl;
			std::cout << "S = " << data->mS << std::endl;
			...
			std::cout << "diff tan x-based rel vel = " << (pred_tan_relvel - true_tan_relvel).cwiseAbs().maxCoeff() << std::endl;
			err = true;
		...
		if (err)
		{
			std::cout << "applied cartesian force = " << applied_force.transpose() << std::endl;
			tVectorXd normal_vel_another = normal_vel_convert_mat * applied_force + normal_vel_convert_vec;
			tVectorXd tangent_vel_another = tangent_vel_convert_mat * applied_force + tangent_vel_convert_vec;
