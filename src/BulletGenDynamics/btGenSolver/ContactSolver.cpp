...
void btGenContactSolver::ConstraintFinished()
{
    ...
    if (mMultibodyArray.size() &&
        mMultibodyArray[0]->GetEnableContactAwareAdviser() == true)
    {
        ...
        {
            contact_torque_array.push_back(new btGenConstraintGeneralizedForce(
                model, dof, control_force_underactuated[dof - 6]));
        }
        // tVectorXd tau = E * Q + Dinv * b;
        // std::cout << "[solved] active tau = " << tau.transpose() <<
        // std::endl;
        // // std::cout << "now Dinvb = " << (Dinv * b).transpose() <<
        // std::endl; for (int dof = 6; dof < num_of_freedom; dof++)
        // {
