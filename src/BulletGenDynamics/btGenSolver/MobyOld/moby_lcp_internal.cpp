...
makeSparseVector(const Eigen::MatrixBase<Derived> &in)
{
    assert(in.cols() == 1);
...
// Sole constructor
moby_lcp_solver::moby_lcp_solver() : log_enabled_(false) {}

void moby_lcp_solver::setLoggingEnabled(bool enabled)
{
    log_enabled_ = enabled;
}

std::ostream &moby_lcp_solver::LOG() const
{
    if (log_enabled_)
    {
        return std::cerr;
...
void moby_lcp_solver::clearIndexVectors() const
{
    // clear all vectors
    _all.clear();
    _tlist.clear();
...
// bool moby_lcp_solver::solve(OptimizationProblem& prog) const {
...
/// nonbasic variables
bool moby_lcp_solver::lcp_fast(const Eigen::MatrixXd &M,
                               ...
                               double zero_tol) const
{
    ...
    LOG() << "moby_lcp_solver::lcp_fast() entered" << std::endl;

    // look for trivial solution
    if (N == 0)
    {
        LOG() << "moby_lcp_solver::lcp_fast() - empty problem" << std::endl;
        z->resize(0);
        return true;
    }
    ...
    if (z->size() == q.size())
    {
        LOG() << "moby_lcp_solver::lcp_fast() - warm starting activated"
              << std::endl;
    ...
    else
    {
        ...
        {
            LOG() << "moby_lcp_solver::lcp_fast() - trivial solution found"
                  << std::endl;
            z->resize(N);
            z->fill(0);
            return true;
    ...
    for (pivots = 0; pivots < MAX_PIV; pivots++)
    {
        ...
        // LOG() << "moby_lcp_solver::lcp_fast() - minimum w after pivot: "
        // << _w[minw] << std::endl;
        ...
        if (minw == UINF || _w[minw] > -zero_tol)
        {
            ...
            {
                LOG() << "moby_lcp_solver::lcp_fast() - minimum z after pivot: "
                      << _z[minz] << std::endl;
            }
            ...
            else
            {
                ...
                LOG() << "moby_lcp_solver::lcp_fast() - solution found!"
                      << std::endl;
                return true;
            }
        }
        else
        {
            ...
            if (minz < UINF && _z[minz] < -zero_tol)
            {
                // move index to basic set and continue looping
                unsigned idx = _nonbas[minz];
                LOG() << "moby_lcp_solver::lcp_fast() - moving index " << idx
    ...
    LOG() << "moby_lcp_solver::lcp_fast() - maximum allowable pivots exceeded"
          << std::endl;
...
bool moby_lcp_solver::lcp_fast_regularized(const Eigen::MatrixXd &M,
                                           ...
                                           double zero_tol) const
{
    ...
    LOG() << "moby_lcp_solver::lcp_fast_regularized() entered" << std::endl;
            ...
            if (_wx.minCoeff() >= -ZERO_TOL)
            {
                ...
                if (wx_min >= -ZERO_TOL && wx_max < ZERO_TOL)
                {
                    ...
                    LOG() << "  pivots / total pivots: " << pivots << " "
                          << pivots << std::endl;
                    LOG() << "moby_lcp_solver::lcp_fast_regularized() exited"
                          << std::endl;
                ...
                {
                    LOG() << "moby_lcp_solver::lcp_fast_regularized() - "
                          << "'<w, z> not within tolerance(min value: "
                          << wx_min << " max value: " << wx_max << ")"
                          << std::endl;
                }
            }
            else
                ...
                if (_wx.minCoeff() > -ZERO_TOL)
                {
                    ...
                    if (wx_min > -ZERO_TOL && wx_max < ZERO_TOL)
                    {
                        ...
                        LOG() << "  pivots / total pivots: " << pivots << " "
                              << total_piv << std::endl;
                        LOG()
                            << "moby_lcp_solver::lcp_fast_regularized() exited"
                            << std::endl;
                        pivots = total_piv;
                        return true;
                    }
    ...
    LOG() << "moby_lcp_solver::lcp_fast_regularized() exited" << std::endl;
...
bool moby_lcp_solver::checkLemkeTrivial(int n, double zero_tol,
                                        const Eigen::VectorXd &q,
...
void moby_lcp_solver::lemkeFoundSolution(const MatrixType &M,
                                         const Eigen::VectorXd &q,
                                         Eigen::VectorXd *z) const
{
 ...
 */
bool moby_lcp_solver::lcp_lemke(const Eigen::MatrixXd &M,
                                const Eigen::VectorXd &q, Eigen::VectorXd *z,
                                double piv_tol, double zero_tol) const
{
    if (log_enabled_)
    {
        LOG() << "moby_lcp_solver::lcp_lemke() entered" << std::endl;
        LOG() << "  M: " << std::endl << M;
        LOG() << "  q: " << q << std::endl;
    }
    ...
    if (checkLemkeTrivial(n, zero_tol, q, z))
    {
        ...
        LOG() << "moby_lcp_solver::lcp_lemke() exited" << std::endl;
        return true;
    }
    ...
    *iiter = t; // replace w var with _z0 in basic indices
    ...
    for (pivots = 0; pivots < MAXITER; pivots++)
    {
        ...
        {
            // if (i == 17)
            //     std::cout << "17 = " << _dl[17] << std::endl;
            if (_dl[i] > PIV_TOL)
            {
                // std::cout << "_j " << _dl[i] << " " << i << " push\n";
                _j.push_back(i);
            }
        }
        ...
        if (_j.empty())
        {
            LOG() << "moby_lcp_solver::lcp_lemke() - no new pivots (ray "
                     "termination)"
                  << std::endl;
            LOG() << "moby_lcp_solver::lcp_lemke() exiting" << std::endl;
            return false;
        }
...
bool moby_lcp_solver::lcp_lemke_regularized(const Eigen::MatrixXd &M,
                                            const Eigen::VectorXd &q,
                                            Eigen::VectorXd *z, int min_exp,
                                            unsigned step_exp, int max_exp,
                                            double piv_tol,
                                            double zero_tol) const
{
    LOG() << "moby_lcp_solver::lcp_lemke_regularized() entered" << std::endl;
    ...
    if (result)
    {
        ...
        if (z->minCoeff() >= -ZERO_TOL)
        {
            ...
            if (_wx.minCoeff() >= -ZERO_TOL)
            {
                ...
                if (wx_min >= -ZERO_TOL && wx_max < ZERO_TOL)
                {
                    LOG() << "  solved with no regularization necessary!"
                          << std::endl;
                    LOG() << "moby_lcp_solver::lcp_lemke_regularized() exited"
                          << std::endl;
            ...
            {
                LOG() << "  moby_lcp_solver::lcp_lemke() - 'w' not solved to "
                         "desired tolerance"
                      << std::endl;
                LOG() << "  minimum w: " << _wx.minCoeff() << std::endl;
            }
        ...
        {
            LOG()
                << "  moby_lcp_solver::lcp_lemke() - 'z' not solved to desired "
                   "tolerance"
                << std::endl;
            LOG() << "  minimum z: " << z->minCoeff() << std::endl;
        }
    ...
    while (rf < max_exp)
    {
        ...
        if (result)
        {
            ...
            if (z->minCoeff() > -ZERO_TOL)
            {
                ...
                if (_wx.minCoeff() > -ZERO_TOL)
                {
                    ...
                    if (wx_min > -ZERO_TOL && wx_max < ZERO_TOL)
                    {
                        LOG()
                            ...
                            << std::endl;
                        LOG()
                            << "moby_lcp_solver::lcp_lemke_regularized() exited"
                            << std::endl;
                        pivots = total_piv;
                        return true;
                    }
                ...
                {
                    LOG()
                        << "  moby_lcp_solver::lcp_lemke() - 'w' not solved to "
                           "desired tolerance"
                        << std::endl;
                    LOG() << "  minimum w: " << _wx.minCoeff() << std::endl;
                }
            ...
            {
                LOG() << "  moby_lcp_solver::lcp_lemke() - 'z' not solved to "
                         "desired tolerance"
                      << std::endl;
                LOG() << "  minimum z: " << z->minCoeff() << std::endl;
            }
    ...
    LOG() << "  unable to solve given any regularization!" << std::endl;
    LOG() << "moby_lcp_solver::lcp_lemke_regularized() exited" << std::endl;
 ...
 *        the solution on output
 */
bool moby_lcp_solver::lcp_lemke(const Eigen::SparseMatrix<double> &M,
                                const Eigen::VectorXd &q, Eigen::VectorXd *z,
                                double piv_tol, double zero_tol) const
{
    if (log_enabled_)
...
bool moby_lcp_solver::lcp_lemke_regularized(
