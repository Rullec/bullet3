...
cBulletPathLCPSolver::cBulletPathLCPSolver()
    : cLCPSolverBase(eLCPSolverType::BulletLemke)
{
    ...
    {
        std::cout
            << "[error] BulletPathLCPSolver create shared memory failed: ";
        switch (errno)
        {
        case EACCES:
            std::cout << "EACCES";
...
int cBulletPathLCPSolver::Solve(int num_of_vars, const tMatrixXd &M,
                                const tVectorXd &n, tVectorXd &x_result)
{
...
void VerifyAnswer(const tMatrixXd &A, const tVectorXd &b, tVectorXd &x)
{
    ...
    // if (true)
    {
        std::cout << "[PATH] verify failed, perp max = "
                  << perp.cwiseAbs().maxCoeff() << std::endl;
        btMathUtil::RoundZero(x, 1e-8);
        btMathUtil::RoundZero(res, 1e-8);
        // std::cout << "[PATH] x = " << x.transpose() << std::endl;

        // // std::cout << "[PATH] x abs max = " << x.cwiseAbs().maxCoeff() <<
        // std::endl;
        // // std::cout << "[PATH] x min = " << x.minCoeff() << std::endl;
        // std::cout << "[PATH] residual = " << res.transpose() << std::endl;
        // // std::cout << "[PATH] residual min = " << res.minCoeff() <<
        // std::endl; std::cout << "[PATH] perp max = " <<
        // perp.cwiseAbs().maxCoeff() << std::endl;
    ...
    {
        // std::cout << "[PATH] LCP solved correct for size " <<
        // memory->num_of_vars
    }
}
