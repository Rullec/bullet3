...
static void swapProblem(ATYPE A, dReal *x, dReal *b, dReal *w, dReal *lo,
                        dReal *hi, int *p, bool *state, int *findex, int n,
                        int i1, int i2, int nskip, int do_fast_row_swaps)
{
    dReal tmpr;
    int tmpi;
    bool tmpb;
...
static void checkFactorization(ATYPE A, dReal *_L, dReal *_d, int nC, int *C,
                               int nskip)
{
    int i, j;
    if (nC == 0)
...
struct dLCP
{
    ...
    dLCP(int _n, int _nskip, int _nub, dReal *_Adata, dReal *_x, dReal *_b,
         dReal *_w, dReal *_lo, dReal *_hi, dReal *_L, dReal *_d, dReal *_Dell,
         dReal *_ell, dReal *_tmp, bool *_state, int *_findex, int *_p, int *_C,
         dReal **Arows);
    ...
    {
        return dDot(AROW(i) + m_nC, q + m_nC, m_nN);
    }
...
dLCP::dLCP(int _n, int _nskip, int _nub, dReal *_Adata, dReal *_x, dReal *_b,
           dReal *_w, dReal *_lo, dReal *_hi, dReal *_L, dReal *_d,
           dReal *_Dell, dReal *_ell, dReal *_tmp, bool *_state, int *_findex,
           int *_p, int *_C, dReal **Arows)
    : m_n(_n), m_nskip(_nskip), m_nub(_nub), m_nC(0), m_nN(0),
      ...
      m_A(_Adata),
#endif
      m_x(_x), m_b(_b), m_w(_w), m_lo(_lo), m_hi(_hi), m_L(_L), m_d(_d),
      m_Dell(_Dell), m_ell(_ell), m_tmp(_tmp), m_state(_state),
      m_findex(_findex), m_p(_p), m_C(_C)
{
    {
        dSetZero(m_x, m_n);
    }
    ...
    */
...

    {
        ...
        for (int k = m_nub; k < n; ++k)
        {
            ...
            {
                swapProblem(m_A, m_x, m_b, m_w, lo, hi, m_p, m_state, findex, n,
                            m_nub, k, m_nskip, 0);
                m_nub++;
            }
        }
    ...
    if (m_findex)
    {
        ...
        for (int k = m_n - 1; k >= nub; k--)
        {
            if (findex[k] >= 0)
            {
                swapProblem(m_A, m_x, m_b, m_w, m_lo, m_hi, m_p, m_state,
...
{
    {
        ...
        swapProblem(m_A, m_x, m_b, m_w, m_lo, m_hi, m_p, m_state, m_findex, m_n,
                    m_nC, i, m_nskip, 1);
               ...
               int nub, dReal *lo, dReal *hi, int *findex)
{
    ...
    dLCP lcp(n, nskip, nub, A, x, b, w, lo, hi, L, d, Dell, ell, delta_w, state,
             findex, p, C, Arows);
        ...
        else
        {
            ...
            for (;;)
            {
                ...
                if (dir > 0)
                {
                    ...
                    {
                        dReal s2 =
                            (hi[i] - x[i]) * dirf; // was (hi[i]-x[i])/dirf	//
                                                   // step to x(i)=hi(i)
                        if (s2 < s)
                        {
                            s = s2;
                ...
                else
                {
                    ...
                    {
                        dReal s2 =
                            (lo[i] - x[i]) * dirf; // was (lo[i]-x[i])/dirf	//
                                                   // step to x(i)=lo(i)
                        if (s2 < s)
                        {
                            s = s2;
...

                {
                    ...
                    for (int k = adj_nub; k < numC; ++k)
                    {
                        ...
                        {
                            dReal s2 = (lo[indexC_k] - x[indexC_k]) /
                                       delta_x[indexC_k];
                            if (s2 < s)
                            {
                                s = s2;
                        ...
                        {
                            dReal s2 = (hi[indexC_k] - x[indexC_k]) /
                                       delta_x[indexC_k];
                            if (s2 < s)
                            {
                                s = s2;
...
size_t dEstimateSolveLCPMemoryReq(int n, bool outer_w_avail)
{
    ...
    // Use n instead of nC as nC varies at runtime while n is greater or equal
    // to nC
...
static size_t EstimateTestSolveLCPMemoryReq(int n)
{
    ...
    res += 10 *
           (sizeof(dReal) * n); // for x, b, w, lo, hi, b2, lo2, hi2, tmp1, tmp2
...
extern "C" ODE_API int dTestSolveLCP()
{
    ...
    // dxAllocateTemporaryWorldProcessMemArena(memreq, NULL, NULL); if (arena ==
    // NULL) {
    //  return 0;
    //}
    ...
    for (int count = 0; count < 1000; count++)
    {
        ...
        */
        ...
        for (i = 0; i < n; i++)
        {
            ...
            {
                dDebug(0, "FAILED: i=%d x=%.4e w=%.4e lo=%.4e hi=%.4e", i, x[i],
                       w[i], lo[i], hi[i]);
            }
        }
