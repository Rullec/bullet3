...
#define M_SQRT1_2 REAL(0.7071067811865475244008443621048490)
     ...
     * fails we want to know where. UASSERT  is a user assertion, i.e. if it
     * fails a nice error message should be printed for the user. AASSERT  is an
     * arguments assertion, i.e. if it fails "bad argument(s)" is printed.
...
#define dDEBUGMSG(msg)                                                         \
    {                                                                          \
        dMessage(d_ERR_UASSERT, msg " in %s() [%s:%u]", __FUNCTION__,          \
                 __FILE__, __LINE__);                                          \
    }
#else // not __GNUC__
#define dIASSERT(a)                                                            \
    {                                                                          \
        if (!(a))                                                              \
        {                                                                      \
            dDebug(d_ERR_IASSERT, "assertion \"" #a "\" failed in %s:%u",      \
                   __FILE__, __LINE__);                                        \
        }                                                                      \
    }
#define dUASSERT(a, msg)                                                       \
...
#error You must #define dSINGLE or dDOUBLE
    ...
    */
    ...
    /* standard joint parameter names. why are these here? - because we don't
     * want to include all the joint function definitions in joint.cpp. hmmmm.
     * MSVC complains if we call D_ALL_PARAM_NAMES_X with a blank second
     * argument, which is why we have the D_ALL_PARAM_NAMES macro as well.
     * please copy and paste between these two.
     */
