...
unsigned long dRandGetSeed() { return seed; }

void dRandSetSeed(unsigned long s) { seed = s; }
...
int dTestRand()
{
    ...
    if (dRand() != 0x3c6ef35f || dRand() != 0x47502932 ||
        dRand() != 0xd1ccf6e9 || dRand() != 0xaaf95334 || dRand() != 0x6252e503)
        ret = 0;
    seed = oldseed;
    return ret;
}
...
int dRandInt(int n)
{
    ...
    // Since there is no memory barrier macro in ODE assign via volatile
    // variable to prevent compiler reusing seed as value of `r'
...
dReal dRandReal() { return ((dReal)dRand()) / ((dReal)0xffffffff); }
