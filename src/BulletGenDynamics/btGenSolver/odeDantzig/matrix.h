...
extern "C"
{
     ...
     * all vectors are n*1. if n <= 0 then zeroes will be returned (in which
     * case the input vectors need not be valid). this function is somewhat
     * faster than calling dDot() for all of the combinations separately.
     */
    ...
    */
    ...
    ODE_API void dMultiply0(dReal *A, const dReal *B, const dReal *C, int p,
                            int q, int r);
    ODE_API void dMultiply1(dReal *A, const dReal *B, const dReal *C, int p,
                            int q, int r);
    ODE_API void dMultiply2(dReal *A, const dReal *B, const dReal *C, int p,
                            int q, int r);
     ...
     * and `d', a n*1 vector of the reciprocal diagonal elements of an n*n
     * matrix D, solve L*D*L'*x=b where x,b are n*1. x overwrites b. the leading
     * dimension of L is `nskip'.
     */

    ODE_API void dSolveLDLT(const dReal *L, const dReal *d, dReal *b, int n,
                            int nskip);
     ...
     *     with ones on the diagonal. only the lower triangle of L is
     * referenced.
     ...
     */
    ODE_API void dLDLTAddTL(dReal *L, dReal *d, const dReal *a, int n,
                            int nskip);
     ...
     *   - L has size n2*n2, its leading dimension in nskip. L is lower
     * triangular with ones on the diagonal. only the lower triangle of L is
     * referenced.
     ...
     * the new L will be written within the old L, i.e. will have the same
     * leading dimension. the last row and column of L, and the last element of
     * d, are undefined on exit.
     *
    ...
    void _dMultiply0(dReal *A, const dReal *B, const dReal *C, int p, int q,
                     int r);
    void _dMultiply1(dReal *A, const dReal *B, const dReal *C, int p, int q,
                     int r);
    void _dMultiply2(dReal *A, const dReal *B, const dReal *C, int p, int q,
                     int r);
    ...
    void _dSolveLDLT(const dReal *L, const dReal *d, dReal *b, int n,
                     int nskip);
    void _dLDLTAddTL(dReal *L, dReal *d, const dReal *a, int n, int nskip,
                     void *tmpbuf);
    void _dLDLTRemove(dReal **A, const int *p, dReal *L, dReal *d, int n1,
                      int n2, int r, int nskip, void *tmpbuf);
