...
int _dFactorCholesky(dReal *A, int n, void *tmpbuf /*[n]*/)
{
    ...
    dReal *recip =
        tmpbuf ? (dReal *)tmpbuf : (dReal *)ALLOCA(n * sizeof(dReal));
    dReal *aa = A;
...
int _dInvertPDMatrix(const dReal *A, dReal *Ainv, int n,
                     void *tmpbuf /*[nskip*(n+2)]*/)
{
    dAASSERT(n > 0 && A && Ainv);
    bool success = false;
    ...
    dReal *tmp = tmpbuf
                     ? (dReal *)tmpbuf
                     : (dReal *)ALLOCA(MaxCholesky_size +
                                       (nskip + nskip_mul_n) * sizeof(dReal));
...
int _dIsPositiveDefinite(const dReal *A, int n, void *tmpbuf /*[nskip*(n+1)]*/)
{
    ...
    dReal *tmp = tmpbuf ? (dReal *)tmpbuf
                        : (dReal *)ALLOCA(FactorCholesky_size +
                                          nskip_mul_n * sizeof(dReal));
...
void _dLDLTRemove(dReal **A, const int *p, dReal *L, dReal *d, int n1, int n2,
                  int r, int nskip, void *tmpbuf /*n2 + 2*nskip*/)
{
...
void dSetZero(dReal *a, int n) { _dSetZero(a, n); }

void dSetValue(dReal *a, int n, dReal value) { _dSetValue(a, n, value); }
...
int dFactorCholesky(dReal *A, int n) { return _dFactorCholesky(A, n, NULL); }
...
void dVectorScale(dReal *a, const dReal *d, int n) { _dVectorScale(a, d, n); }
...
void dLDLTRemove(dReal **A, const int *p, dReal *L, dReal *d, int n1, int n2,
                 int r, int nskip)
{
