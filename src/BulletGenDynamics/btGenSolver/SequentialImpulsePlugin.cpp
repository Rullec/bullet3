...
void btGenContactSolver::SolveBySI()
{
    ...
    // 1. set up SI data, calculate the convert mat from cartesian force to
    // relative velocity, which will be used in the iterations
    SetupDataForSI();
    // do something, velocities are updated during SI, but transforms are kept
    // the same.
    int iter = 0;
    // std::cout << "-----------outut contact info begin-----------\n";
    // std::cout << "contact size = " << mContactConstraintData.size() <<
    // std::endl; if (mContactConstraintData.size() == 3)
    // {
    // 	std::cout << "3 contacts\n";
    // }
    ...
    // 		std::cout << "for contact " << i << ", bodyA = " <<
    // data->mBodyA->GetName() << ", bodyB = " << data->mBodyB->GetName() <<
    // std::endl; 		std::cout << "contact normal = " <<
    ...
    for (iter = 0; iter < mMaxItersSI; iter++)
    {
        ...
        if (iter % 100 == 0 && iter && mEnableDebugOutput)
            std::cout << "iter " << iter << " lease square = " << leaseSquare
                      << std::endl;
        if (leaseSquare < mConvergeThresholdSI)
        {
            if (mEnableDebugOutput)
            {
                std::cout << "[SI] convergence after " << iter
                          << " iterations\n";
            }
    ...
    if (iter == mMaxItersSI)
        std::cout << "[SI] still didn't convergence after max " << mMaxItersSI
                  << " iterations\n";
    PopState("before_SI");
    ...
    for (int i = 0; i < mContactConstraintData.size(); i++)
    {
        ...
        x_si.segment(i * 3, 3) =
            (data->mSI_normal_contact_value * data->mNormalPointToA +
             data->mSI_tangent_contact_value[0] *
                 data->mSI_tangent_contact_dir[0] +
             data->mSI_tangent_contact_value[1] *
                 data->mSI_tangent_contact_dir[1])
                .segment(0, 3);
    }
}
...
double btGenContactSolver::InternalIterationSI()
{
    ...
    for (auto &data : mContactConstraintData)
    {
        ...
        double delta_normal_force =
            ...
            ((data->mNormalPointToA.transpose() * data->mSI_Z)
                 .dot(data->mNormalPointToA));
        ...
        if (mEnableFrictionalSI)
        {
            double delta_tangent_force_1 =
                ...
                ((data->mSI_tangent_contact_dir[0].transpose() * data->mSI_Z)
                     .dot(data->mSI_tangent_contact_dir[0]));

            double delta_tangent_force_2 =
                -data->mSI_tangent_contact_dir[1].dot(data->mSI_h) /

                ((data->mSI_tangent_contact_dir[1].transpose() * data->mSI_Z)
                     .dot(data->mSI_tangent_contact_dir[1]));
            ...
            double new_friction_norm =
                std::sqrt(std::pow(new_tangent_force_total_1, 2) +
                          std::pow(new_tangent_force_total_2, 2));

            if (std::fabs(new_friction_norm) >=
                mMu * data->mSI_normal_contact_value)
            {
                // dynamics friction
                new_tangent_force_total_1 /=
                    (std::fabs(new_friction_norm) /
                     (mMu * data->mSI_normal_contact_value));
                new_tangent_force_total_2 /=
                    (std::fabs(new_friction_norm) /
                     (mMu * data->mSI_normal_contact_value));

                delta_tangent_force_1 = new_tangent_force_total_1 -
                                        data->mSI_tangent_contact_value[0];
                delta_tangent_force_2 = new_tangent_force_total_2 -
                                        data->mSI_tangent_contact_value[1];
            }
            // std::cout << "delta tan1 = " << delta_tangent_force_1 << ", 2 = "
            // << delta_tangent_force_2 << std::endl;
            total_force +=
                delta_tangent_force_1 * data->mSI_tangent_contact_dir[0] +
                delta_tangent_force_2 * data->mSI_tangent_contact_dir[1];

            total_tangent_changed += std::fabs(delta_tangent_force_1) +
                                     std::fabs(delta_tangent_force_2);
        }

        // dry friction
        // std::cout << "[inner iter] for contact " << contact_id << std::endl;
        // std::cout << "normal = " << data->mNormalPointToA.transpose() <<
        // std::endl; std::cout << "SI_Z = " << data->mSI_Z << std::endl;
        // std::cout << "SI_h = " << data->mSI_h.transpose() << std::endl;
        // std::cout << "delta normal force = " << delta_normal_force <<
        // std::endl;
        data->ApplyForceCartersian(total_force);
        // std::cout << "apply " << (delta_normal_force *
        // data->mNormalPointToA).transpose() << std::endl;
        {
...
void btGenContactSolver::SetupDataForSI()
{
    ...
    for (auto &data : mContactConstraintData)
    {
        ...
        data->mSI_tangent_contact_value[0] =
            data->mSI_tangent_contact_value[1] = 0;
        data->mSI_Z.setZero();
        data->mSI_h.setZero();

        // 2. set up two perpendicular directions
        tMatrixXd FrictionDir4 =
            btMathUtil::ExpandFrictionCone(4, data->mNormalPointToA);
        // std::cout << FrictionDir4.rows() << " " << FrictionDir4.cols() <<
        // std::endl;
        ...
        auto &body0_A_mat =
            mColGroupData[body0_groupid]->mConvertCartesianForceToVelocityMat;
        auto &body1_A_mat =
            mColGroupData[body1_groupid]->mConvertCartesianForceToVelocityMat;
        auto &body0_b_vec =
            mColGroupData[body0_groupid]->mConvertCartesianForceToVelocityVec;
        auto &body1_b_vec =
            mColGroupData[body1_groupid]->mConvertCartesianForceToVelocityVec;
        // std::cout << "for contact " << contact_id << std::endl;

        // shape the mSI_Z and mSI_h
        data->mSI_Z.block(0, 0, 3, 3) =
            body0_A_mat.block(contact_id * 3, contact_id * 3, 3, 3) -
            body1_A_mat.block(contact_id * 3, contact_id * 3, 3, 3);

        // std::cout << "bodyA vec_b = " << body0_b_vec.transpose() <<
        // std::endl; std::cout << "bodyB vec_b = " << body1_b_vec.transpose()
        // << std::endl;
        data->mSI_h.segment(0, 3) = body0_b_vec.segment(contact_id * 3, 3) -
                                    body1_b_vec.segment(contact_id * 3, 3);
...
bool btGenContactSolver::IsMultibodyAndVelMax(btGenCollisionObject *body)
{
    ...
    {
        std::cout << "[verify nt rel vel] bodyB now qdot is up to max_vel, so "
                     "it's trivial to verify failed(caused by clamp)\n";
        std::cout << "qdot = "
                  << UpcastRobotCollider(body)->mModel->Getqdot().transpose()
                  << std::endl;
        return true;
    }
    else
    ...
    for (int i = 0; i < mContactConstraintData.size(); i++)
    {
        if (i == 0)
            std::cout << "----------begin to compare result between LCP and "
                         "SI-----------\n";
        std::cout << "contact " << i
                  << " lcp force = " << f_lcp.segment(i * 3, 3).transpose()
                  << std::endl;
        std::cout << "contact " << i
                  << " SI force = " << x_si.segment(i * 3, 3).transpose()
                  << std::endl;
    }
}