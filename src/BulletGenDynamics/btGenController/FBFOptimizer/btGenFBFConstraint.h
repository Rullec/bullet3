...
public:
	...
	virtual void AddEqCon(const tMatrixXd &Jacobian, const tVectorXd &residual, int st_pos);
	virtual void AddEquivalentEqCon(const tMatrixXd &Jacobian, const tVectorXd &residual,
									int st_pos, const double tolerance);

	// Jacobian * x + residual >= 0
	virtual void AddIneqCon(const tMatrixXd &Jacobian, const tVectorXd &residual,
							int st_pos);
