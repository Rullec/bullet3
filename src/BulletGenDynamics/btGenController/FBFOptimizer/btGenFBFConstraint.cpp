...
btGenFrameByFrameConstraint::btGenFrameByFrameConstraint(int size) : mSolutionSize(size)
{
...
int btGenFrameByFrameConstraint::GetNumOfEqCon() const { return mNumOfEqConstraint; }
int btGenFrameByFrameConstraint::GetNumOfIneqCon() const { return mNumOfIneqConstraint; }

/**
 * jac * x + residual = 0
*/
void btGenFrameByFrameConstraint::AddEqCon(const tMatrixXd &jac, const tVectorXd &residual, int st_pos)
{
 ...
 */
void btGenFrameByFrameConstraint::AddEquivalentEqCon(const tMatrixXd &jac, const tVectorXd &residual,
													 int st_pos, const double tolerance)
{
	assert(tolerance >= 0);
	// jac * x = residual

	// to 1. jac * x >= residual - tol
	AddIneqCon(jac, residual - tolerance * tVectorXd::Ones(residual.size()), st_pos);
...
void btGenFrameByFrameConstraint::AddIneqCon(const tMatrixXd &jac, const tVectorXd &residual, int st_pos)
{
	Validate(jac, residual, st_pos);
...
void btGenFrameByFrameConstraint::Validate(const tMatrixXd &jac, const tVectorXd &res, int st)
{
	int jac_size = jac.rows(), res_size = res.size();
	if (jac_size != res_size)
	{
		std::cout << "btGenFrameByFrameConstraint::Validate: the size of jacobian %d "
	...
	{
		std::cout << "btGenFrameByFrameConstraint::Validate: constraint start pos %d is illegal, "
					 "constraint length %d, solution size %d\n";
		exit(1);
	}
}

void btGenFrameByFrameConstraint::GetEqJacobianAndResidual(tMatrixXd &Jac_total, tVectorXd &res_total)
{
...
void btGenFrameByFrameConstraint::GetIneqJacobianAndResidual(tMatrixXd &Jac_total, tVectorXd &Res_total)
{
