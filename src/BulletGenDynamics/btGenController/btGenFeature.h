...
/**
 * \brief			A btGenFeature is a consist element in the total feature
 * vector.
 *
 * 		It can be:
 * 			1. joint angle
...
/**
 * \brief				This class construct the feature vector in
 * contact-aware LCP controlk
 */
...
public:
    ...
    btGenFeatureArray();
    void Init(const std::string &conf, cRobotModelDynamics *model, btTraj *traj,
              const tVector &g);
    // void Eval(const int target_frame, tMatrixXd& H, tMatrixXd& E, tVectorXd&
    // f);
    void Eval(const tVectorXd &qddot_target, const tVectorXd &tau_target,
              tMatrixXd &H, tMatrixXd &E, tVectorXd &f);
    ~btGenFeatureArray();
...
protected:
    ...
    tMatrixXd mD_tau,
        mM_Q; // convert mat from tau/Q_\chi to qddot, \chi is the contact force
    tVectorXd mn_tau; // convert residual from tau/Q_\chi to qddot, \chi is the
                      // contact force
    //--------------methods
    void InitWeight(const std::string &conf);
    void InitFeature(const std::string &conf);
    // void InitRefFeature();
    void EvalConvertMatAndResidual(tMatrixXd &C_alpha,
                                   tVectorXd &d_alpha) const;
    void EvalPrerequistite(tMatrixXd &D_tau, tMatrixXd &M_Q,
                           tVectorXd &n_tau) const;
    void EvalDynamicTerms(const tVectorXd &ref_feature,
