...
{
	// std::cout << "J.adjoint size = " << (J.adjoint().rows()) << " "
	// 		  << (J.adjoint().cols()) << std::endl;
	// std::cout << "mp size = " << np.size() << std::endl;
	d = J.adjoint() * np;
}
							 ...
							 const VectorXd &ci0, VectorXd &x)
{
	...
	double t, t1, t2; /* t is the step length, which is the minimum of the
						   ...
						   double &R_norm)
{
	...
	for (j = n - 1; j >= iq + 1; j--)
	{
		/* The Givens rotation is done with the matrix (cc cs, cs -cc).
                             If cc is one, then element (j) of d is zero
           compared with element (j - 1). Hence we don't have to do anything. If
           cc is zero, then we just have to switch column (j) and column (j - 1)
                             of J. Since we only switch columns in J, we have to
           be careful how we update d depending on the sign of gs. Otherwise we
           have to apply the Givens rotation to these columns.
