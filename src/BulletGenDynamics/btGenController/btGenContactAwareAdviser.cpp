...
// btGenContactAwareAdviser::btGenContactAwareAdviser(btGeneralizeWorld* world, const std::string& path, double W, double Wm) : mW(W), mWm(Wm)
// std::string new_path = "new.out";
btGenContactAwareAdviser::btGenContactAwareAdviser(btGeneralizeWorld* world, const std::string& config)
{
	mModel = nullptr;
...
void btGenContactAwareAdviser::Init(cRobotModelDynamics* model)
{
	...
	if (mModel != nullptr)
	{
		std::cout << "[error] btGenContactAwareAdviser: model is not empty, exit\n";
		exit(1);
	}
	mModel = model;
	...
	LoadTraj(mTrajPath);
	mFeatureVector->Init(mFeatureVectorFile, mModel, mRefTraj, mWorld->GetGravity());
...
void btGenContactAwareAdviser::Update(double dt)
{
	...
	{
		std::cout << "[error] btGenContactAwareAdviser ref motion dt " << mRefTraj->mTimestep << " != sim dt " << dt << std::endl;
		exit(1);
	}
	if (IsEnd() == true)
	{
		std::cout << "end, save the new recorded traj to " << mSavedTrajPath << std::endl;
	...
	// 	// std::cout << "[ref] qdot diff = " << qdot_diff.transpose() << std::endl;
	...
	UpdateRefChar();
	// std::cout << "[debug] [new] frame " << mInternalFrameId << " N norm = " << mN.norm() << std::endl;
	// std::cout << "[debug] [new] frame " << mInternalFrameId << " H norm = " << mH.norm() << std::endl;
	// std::cout << "[debug] [new] frame " << mInternalFrameId << " E norm = " << mE.norm() << std::endl;
	// std::cout << "[debug] [new] frame " << mInternalFrameId << " f norm = " << mf.norm() << std::endl;
	mInternalFrameId++;
...
/**
 * \brief					Resolve the active force by the qddot and contact forces
*/
void btGenContactAwareAdviser::ResolveActiveForce()
{
	...
	for (int frame_id = 1; frame_id < num_of_frame - 1; frame_id++)
	{
		...
		for (const auto& fc : fcs)
		{
			...
			mModel->ComputeJacobiByGivenPointTotalDOFWorldFrame(link_id, fc->mWorldPos.segment(0, 3), jac);
...
/**
 * \brief					Init the pose of guided character by the input trajectory
*/
void btGenContactAwareAdviser::PostProcess()
{
	...
	mModel->SetqAndqdot(mRefTraj->mq[mInternalFrameId], mRefTraj->mqdot[mInternalFrameId]);
	std::cout << "[adviser] init q = " << mRefTraj->mq[mInternalFrameId].transpose() << std::endl;
	std::cout << "[adviser] init qdot = " << mRefTraj->mqdot[mInternalFrameId].transpose() << std::endl;
	if (mModel->GetNumOfFreedom() != mRefTraj->mq[mInternalFrameId].size())
	{
		std::cout << "btGenContactAwareAdviser::PostProcess the model is inconsistent with the given motion\n";
		exit(1);
	}
...
/**
 * \brief					input the contact forces (generalized), calculate and apply the active control torque
*/
tVectorXd btGenContactAwareAdviser::CalcControlForce(const tVectorXd& Q_contact)
{
	...
	// std::cout << "[control] contact force norm = " << Q_contact.norm() << std::endl;
	// std::cout << "[control] mH norm = " << mH.norm() << std::endl;
	...
	tVectorXd ref_force = mRefTraj->mActiveForce[mInternalFrameId - 1].transpose().segment(6, mModel->GetNumOfFreedom() - 6);
	// std::cout << "q = " << mModel->Getq().segment(0, 3).transpose() << std::endl;
	// std::cout << "[adviser] ref ctrl force = " << ref_force.transpose() << std::endl;
	// std::cout << "[adviser] calced ctrl force = " << Q_active.transpose() << std::endl;

	if (mEnableRecordCtrledTraj == true)
	{
		mSavedTraj->mActiveForce[mInternalFrameId - 1] = tVectorXd::Zero(num_of_freedom);
		mSavedTraj->mActiveForce[mInternalFrameId - 1].segment(6, num_of_underactuated_freedom) = Q_active;
	}
	// if (Q_active.norm() > 1e8)
	// {
	// 	std::cout << "[warn] force explode, use the reference torque = " << ref_force.transpose() << std::endl;
	// 	Q_active = ref_force;
	// }
	return Q_active;
}
...
void btGenContactAwareAdviser::ReadConfig(const std::string& config)
{
	...
	mResolveControlToruqe = btJsonUtil::ParseAsBool("resolve_control_tau", root);
	mOutputControlDiff = btJsonUtil::ParseAsBool("output_diff", root);
	mEnableFrameByFrameCtrl = btJsonUtil::ParseAsBool("enable_frame_by_frame_control", root);
	mOutputControlDiffFile = btJsonUtil::ParseAsString("output_diff_path", root);
	mDrawReferenceTrajCharacter = btJsonUtil::ParseAsBool("draw_reference_traj_character", root);
	mDrawTargetFBFCharacter = btJsonUtil::ParseAsBool("draw_target_FBF_character", root);
	...
	mCtrlConf = btJsonUtil::ParseAsValue("ctrl_config", root);
	mEnableRecordCtrledTraj = btJsonUtil::ParseAsBool("enable_record_ctrled_traj", root);
	mSavedTraj = nullptr;
	...
	{
		std::cout << "[error] ContactAwareAdviser::Readconfig: 2 draw options are both turned on, exit\n";
		exit(1);
	}
}
...
void btGenContactAwareAdviser::UpdateMultibodyVelocityDebug(double dt)
{
	...
	if (mOutputControlDiff == true)
	{
		...
		// fout << "[ref] qddot diff = " << (cur_qddot - ref_qddot).transpose() << std::endl;
		// fout.close();
		// std::cout << "[ref] qddot = " << ref_qddot.transpose() << std::endl;
		// std::cout << "[true] qddot = " << cur_qddot.transpose() << std::endl;
		tVectorXd q_diff = mModel->Getq() - mRefTraj->mq[mInternalFrameId - 1],
				  qdot_diff = mModel->Getqdot() - mRefTraj->mqdot[mInternalFrameId - 1];
		tVectorXd qddot_diff = cur_qddot - ref_qddot;
		std::cout << "[true] qddot diff = " << qddot_diff.norm() << ", qdot diff = " << qdot_diff.norm() << ", q diff = " << q_diff.norm() << std::endl;
	}

	mModel->UpdateVelocity(dt, false);
}
/**
 * \brief					Update the multibody velocity, used in debug mode
*/
void btGenContactAwareAdviser::UpdateMultibodyVelocityAndTransformDebug(double dt)
{
	tVectorXd cur_qddot = mModel->Getqddot();
	tVectorXd ref_qddot = mRefTraj->mqddot[mInternalFrameId - 1];
	if (mOutputControlDiff == true)
...
tVectorXd btGenContactAwareAdviser::CalcLCPResidual(double dt) const
{
	...
	tVectorXd residual = dt * Minv * (QG + mN * mE.inverse() * mf - mModel->GetCoriolisMatrix() * qdot) + qdot;
	// std::cout << "[debug] [new] frame " << mInternalFrameId << " residual = " << residual.transpose() << std::endl;
	return residual;
}
tMatrixXd btGenContactAwareAdviser::CalcLCPPartBPrefix() const
{
	...
	int num_of_freedom = mModel->GetNumOfFreedom();
	tMatrixXd partb = tMatrixXd::Identity(num_of_freedom, num_of_freedom) + mN * mH;
	// std::cout << "[debug] [new] frame " << mInternalFrameId << " PartB norm = " << partb.norm() << std::endl;
	// std::cout << "[debug] [new] frame " << mInternalFrameId << " PartB mN = " << mN.norm() << std::endl;
	// std::cout << "[debug] [new] frame " << mInternalFrameId << " PartB mE = " << mH.norm() << std::endl;
	return partb;
...
/**
 * \brief					Get current reference gen accel "qddot_target" and "tau_target"
*/
void btGenContactAwareAdviser::GetTargetInfo(double dt, tVectorXd& qddot_target, tVectorXd& tau_target)
{
	if (false == mEnableFrameByFrameCtrl)
	{
		...
		tau_target = mRefTraj->mActiveForce[mInternalFrameId].segment(6, num_of_underactuated_freedom);
	}
	else
	{
		mFBFOptimizer->CalcTarget(dt, mInternalFrameId, qddot_target, tau_target);
	}
}

/**
 * \brief					Create the reference character in ref trajectory
*/
void btGenContactAwareAdviser::CreateRefChar()
{
	mRefModel = new cRobotModelDynamics();

	mRefModel->Init(mModel->GetCharFile().c_str(), mModel->GetScale(), ModelType::JSON);
	mRefModel->InitSimVars(mWorld->GetInternalWorld(), true, true, false);
	// mRefModel->SetqAndqdot(mRefTraj->mq[mInternalFrameId], mRefTraj->mqdot[mInternalFrameId]);
}
...
/**
 * \brief					record the current contact info and motion status into the mSaveTraj
*/
void btGenContactAwareAdviser::RecordTraj()
{
	// record motion
	mSavedTraj->mq[mInternalFrameId] = mModel->Getq();
	// std::cout << "record traj " << mInternalFrameId << " " << mSavedTraj->mq[mInternalFrameId].transpose() << std::endl;
	// record contacts

	{
		// const std::vector<btGenContactForce*>& cur_contacts =;
		auto& rec_contacts = mSavedTraj->mContactForce[mInternalFrameId - 1];
		for (auto& x : rec_contacts) delete x;
		rec_contacts.clear();
		int num_of_contact_forces = mWorld->GetContactForces().size();
		std::cout << "[rec] contact num = " << num_of_contact_forces << std::endl;
		for (int id = 0; id < num_of_contact_forces; id++)
		{
			...
			{
				rec_contacts.push_back(new btGenContactForce(cur_contact->mObj, cur_contact->mForce, cur_contact->mWorldPos, cur_contact->mIsSelfCollision));
			}
		}
	}
	// exit(1);
