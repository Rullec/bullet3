...
{
	return type >= btGenFeatureType::SphereJoint && type <= btGenFeatureType::FixedJoint;
}
bool IsPosFeature(btGenFeatureType type)
{
	return type >= btGenFeatureType::Height && type <= btGenFeatureType::Location;
}
...
std::string GetFeatureSuffix(const std::string& name)
{
	...
	if (st == -1)
	{
		std::cout << "[error] GetFeatureSuffix _ not found in " << name << std::endl;
		exit(1);
	}
...
std::string GetFeatureLinkName(const std::string& name)
{
	...
	if (st == -1)
	{
		std::cout << "[error] GetFeatureLinkName _ not found in " << name << std::endl;
		exit(1);
	}
...
btGenFeatureArray::~btGenFeatureArray()
{
	for (auto& x : mFeatureVector) delete x;
}
...
*/
void btGenFeatureArray::Init(const std::string& conf, cRobotModelDynamics* model, btTraj* traj, const tVector& g)
{
	mModel = model;
	...
	// init mN matrix
	{
		...
		mN.block(6, 0, num_of_underactuated_freedom, num_of_underactuated_freedom).setIdentity();
	}
...
void btGenFeatureArray::InitFeature(const std::string& conf)
{
	...
	// std::cout << "before total feature size = " << mTotalFeatureSize << std::endl;
	for (int i = 0; i < keys.size(); i++)
	{
		...
		{
			std::cout << "[error] unsupported feature suffix = " << suffix << std::endl;
			exit(1);
		}
	...
	// exit(1);
	// 2. sort the feature according to link id, because the json file has an random key order
	// then build the feature offset (the begin position of a specieifed feature in a vector)
	...
	// 3. validate the feature array: are all dof controlled by this feature? If not, exit
	PrintFeatureInfo();
...
/**
 * \brief						Evaluate in this reference frame, get the feature matrix H, E, f whicah can be used later
 * 
 * 1. Given target active force and target qddot, calculate target feature vector:
 * 
 * 2. Calculate current dynamic terms:
 * 	H = -E^{-1} * D_\tau^T * W^T * W * M_Q
 * 	E = D_\tau^T * W^T * W * M_\tau + W_m^T * W_m
 *  f = W_m^T * W_m \tilde{\tau} - D_\tau^T * W^T * W * M_Q * (n_\tau - \tilde{\alpha})
 * 
 * 3. Prerequisite:
...
*/
void btGenFeatureArray::Eval(const tVectorXd& qddot_target, const tVectorXd& tau_target, tMatrixXd& H, tMatrixXd& E, tVectorXd& f)
{
	// if (target_frame >= mTraj->mNumOfFrames - 2)
	// {
	// 	std::cout << "[error] void btGenFeatureArray::Eval frame " << target_frame << " exceeds the range\n";
	// 	exit(1);
	// }
	...
	// std::cout << "ref feature vector " << mRefFeature[target_frame].transpose() << std::endl;
	// std::cout << "mH = \n"
	// 		  << H << std::endl;
	// exit(1);
}
...
{
	if (btGenFeatureType::INVALID_FEATURE_TYPE == feature->mFeatureType || feature->mFeatureType >= btGenFeatureType::NUM_OF_GEN_FEATURE_TYPE)
	{
...
*/
btGenFeature* btGenFeatureArray::CreateJointFeature(const std::string& joint_name) const
{
	...
	switch (joint->GetJointType())
	{
		...
		case JointType::FIXED_JOINT:
			...
			break;
		default:
			std::cout << "[error] unsupported joint type " << joint->GetJointType() << std::endl;
			exit(1);
			break;
	}
	// std::cout << "pu"sh " << joint_name << " type " << joint->GetJointType() << " id " << feature->mLinkId << std::endl;

	return feature;
}

btGenFeature* btGenFeatureArray::CreateHeightFeature(const std::string& link_name) const
{
...
btGenFeature* btGenFeatureArray::CreateLocationFeature(const std::string& link_name) const
{
	BaseObject* link = mModel->GetLink(link_name);
	btGenFeature* feature = new btGenFeature();
	feature->mLinkId = link->GetId();
...
void btGenFeatureArray::PrintFeatureInfo() const
{
	...
	{
		// std::cout << "feature " << x->mLinkName << " type " << gGenFeatureTypeStr[x->mFeatureType] << " size " << GetSingleFeatureSize(x) << " order " << x->mFeatureOrder
		// 		  << " weight " << x->mWeight << std::endl;
		if (x->mFeatureType >= btGenFeatureType::SphereJoint && x->mFeatureType <= btGenFeatureType::FixedJoint && x->mFeatureOrder == 2)
		{
		...
		else
		{
			switch (x->mFeatureType)
			{
				case btGenFeatureType::Height:
					std::cout << "link " << x->mLinkName << " height is controlled, weight = " << x->mWeight << std::endl;
					break;
				case btGenFeatureType::Location:
					std::cout << "link " << x->mLinkName << " location is controlled, weight = " << x->mWeight << std::endl;
					break;
				default:
					continue;
					break;
	...
	for (int link_id = 0; link_id < num_of_links; link_id++)
	{
		...
		{
			std::cout << "[error] link " << link_id << " " << mModel->GetLinkById(link_id)->GetName() << " is not controlled\n";
			exit(1);
		}
	}
...
#include "BulletGenDynamics/btGenModel/Link.h"
void btGenFeatureArray::TestdJvdq(tVectorXd& q, tVectorXd& qdot, tVectorXd& qddot, int link_id)
{
	...
	for (int dof = 0; dof < num_of_freedom; dof++)
	{
		...
		if (diff_norm > eps)
		{
			std::cout << "[error] link " << link_id << " dof " << dof << " diff norm = " << diff_norm << std::endl;
			exit(1);
		}
	}
 ...
 * 
 * 	Feature vector is consist of generalized accel or cartesian accel of any link COM.
*/
void btGenFeatureArray::EvalConvertMatAndResidual(tMatrixXd& C_alpha, tVectorXd& d_alpha) const
{
	int num_of_freedom = mModel->GetNumOfFreedom();
	...
	for (int id = 0; id < mNumOfFeature; id++)
	{
		...
		switch (feature->mFeatureType)
		{
			...
			case btGenFeatureType::Location:
			{
				...
				C_alpha.block(feature_offset, 0, feature_size, num_of_freedom) = mConvertPosToXZ * Jv;
				d_alpha.segment(feature_offset, feature_size) = mConvertPosToXZ * Jv_dot * qdot;
			...
			case btGenFeatureType::Height:
			{
				...
				C_alpha.block(feature_offset, 0, feature_size, num_of_freedom) = mConvertPosToY * Jv;
				d_alpha.segment(feature_offset, feature_size) = mConvertPosToY * Jv_dot * qdot;
			}
			break;
			default:
				std::cout << "[error] Unsupported feature type " << feature->mFeatureType, exit(1);
				break;
		}
	}
...
/**
 * \brief					Calculate the convert mat/convert residual from the control torque \tau to qddot
 *
 ...
 *  
*/
void btGenFeatureArray::EvalPrerequistite(tMatrixXd& D_tau, tMatrixXd& M_Q, tVectorXd& n_tau) const
{
	...
	n_tau.noalias() = md_alpha + mC_alpha * Minv * (Q_G - mModel->GetCoriolisMatrix() * mModel->Getqdot());
	// n_tau.noalias() = md_alpha + mC_alpha * Minv * Q_G - mC_alpha * Minv * mModel->GetCoriolisMatrix() * mModel->Getqdot();
}

/**
 * \brief						Calculate the dynamic terms, matrices E, H and vector f
 * 	
 * 	E = D_\tau^T * W^T * W * M_\tau + W_m^T * W_m
 *  H = -E^{-1} * D_\tau^T * W^T * W * M_Q
 *  f = W_M^T * W_m * \tilde{\tau} - D_\tau^T * W^T * W *(n_\tau - \tilde{\alpha})
*/
// #include <fstream>
// extern std::string new_path;
void btGenFeatureArray::EvalDynamicTerms(const tVectorXd& ref_feature, const tVectorXd& ref_tau, tMatrixXd& H, tMatrixXd& E, tVectorXd& f) const
{
	...
	f.noalias() = WmTWm * ref_tau - mD_tau.transpose() * WTW * mn_tau + mD_tau.transpose() * WTW * ref_feature;

	// const tVectorXd& ref_tau = mTraj->mActiveForce[target_frame].segment(6, num_of_underactuated_freedom);
	// const tVectorXd& ref_feature = mRefFeature[target_frame];
	// {
	...
	// std::cout << "[eval] C norm = " << (mD_tau.transpose() * WTW).norm() << std::endl;
	// std::cout << "[eval] Minv norm = " << (mM_Q).norm() << std::endl;
	// std::cout << "[eval] C norm = " << (mD_tau.transpose() * WTW).norm() << std::endl;
	// std::cout << "[eval] ref tau norm = " << ref_tau.norm() << std::endl;
	// std::cout << "[eval] ref feature norm = " << ref_feature.norm() << std::endl;
	// std::cout << "[eval] n_tau norm = " << mn_tau.norm() << std::endl;
	// std::cout << "[eval] D_tau norm = " << mD_tau.norm() << std::endl;
	//

	{
		//
		// f.noalias() = WmTWm * ref_tau - mD_tau.transpose() * WTW * (mn_tau - ref_feature);
	} {
		// sometimes, n_tau and ref feature are all quite small. Multiply them dividely can reduce the truncation error
	...
	// tVectorXd f_p24 = -mD_tau.transpose() * WTW * mn_tau + mD_tau.transpose() * WTW * ref_feature;
...
/**
 * \brief					Given the gen accel qddot, calculate current target feature vector
*/
tVectorXd btGenFeatureArray::CalcTargetFeature(const tVectorXd& qddot)
{
	// std::cout << "init ref feature begin " << mTraj->mNumOfFrames << std::endl;
	// whatever feature type is defined, we only control from 1 - N-2
	...
	for (int fea_id = 0; fea_id < mNumOfFeature; fea_id++)
	{
		...
		{
			std::cout << "[error] InitReffeature feature " << fea_id << "is not 2-order(accel), exit\n";
			exit(1);
		}

		// for joint feature, the feature vector is simply genearlized coordinate
		...
		else if (IsPosFeature(feature->mFeatureType))
		{
			...
			auto link = dynamic_cast<Link*>(mModel->GetLinkById(feature->mLinkId));
			tVector3d accel = tVector3d::Zero();
			accel += link->GetJKv() * qddot;
			for (int dof = 0; dof < mModel->GetNumOfFreedom(); dof++) accel + link->GetTotalDofdJKv_dq(dof) * qdot;
			...
			switch (feature->mFeatureType)
			{
				...
				default:
					std::cout << "[error] illegal feature type " << feature->mFeatureType << std::endl, exit(1);
					break;
			}
		}
		else
		{
			std::cout << "[error] Neither joint nor pos feature, illegal feature type " << feature->mFeatureType << std::endl, exit(1);
		}
	}
