...
/**
 * \brief               In contact-aware character control, the LCP problem is resposible to both the contact force and the control forces
 * This class tries to provide a motion guidence (the acceleration of character) in current frame
*/
struct btTraj;
...
public:
	...
	// btGenContactAwareAdviser(btGeneralizeWorld* world, const std::string& guide_traj, double W, double Wm);
	btGenContactAwareAdviser(btGeneralizeWorld* world, const std::string& config);
	...
	// void GetAdviseInfo(tMatrixXd& C, tMatrixXd& D, tMatrixXd& E, tMatrixXd& N, tVectorXd& b);
...
protected:
	...
	tMatrixXd mH, mE;                  // convert matrices, please check the Note for more details
	...
	void GetTargetInfo(double dt, tVectorXd& qddot_target, tVectorXd& tau_target);
