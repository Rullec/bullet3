...
void btGenPDController::ApplyGeneralizedTau(double timestep)
{
	...
	for (int i = 0; i < dof; i++)
	{
		// std::cout << "qdot " << i << " = " << qdot[i] << ", max vel = " << mModel->GetMaxVel() << std::endl;
		// std::cout << "res = " << (std::fabs(qdot[i]) - 10) << std::endl;
		if ((std::fabs(qdot[i])) >= mModel->GetMaxVel() - 5)
		{
			std::cout << "qdot " << i << " = " << qdot[i] << " exceed the maxvel, belongs to joint " << mModel->GetJointByDofId(i)->GetName() << std::endl;
		}
	}
	if (mEnableSPD == false)
	{
		tau = mKp.cwiseProduct(mTargetq - q) + mKd.cwiseProduct(mTargetqdot - qdot);
	}
	else
	{
		tau =
			-mKp.cwiseProduct(q + timestep * qdot - mTargetq) - mKd.cwiseProduct(qdot + timestep * qddot - mTargetqdot);
	}

	double max_tau = tau.cwiseAbs().maxCoeff();
	if (max_tau > mTorqueLim)
	{
		std::cout << "the computed control torque " << max_tau << " > " << mTorqueLim << ", clamped!";
	...
	for (int i = 0; i < dof; i++) mModel->ApplyGeneralizedForce(i, tau[i]);
