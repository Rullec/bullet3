	...
	for (int i = 0; i < model->GetNumOfJoint(); i++)
	{
		...
		switch (type)
		{
			...
			case JointType::NONE_JOINT:
				...
				break;
			default:
				std::cout << "get model pose size unsuporeted joint type " << type << std::endl;
				exit(1);
				break;
		}
	}
...
/**
 * \brief				Convert generalized coordinate q to char pose
 * 
*/
tVectorXd ConvertqToPose(const tVectorXd& q, cRobotModelDynamics* model)
{
	...
	{
		std::cout << "[error] convert q to pose illegal, size = " << q.size() << " " << model->GetNumOfFreedom() << std::endl;
		exit(1);
	}
		...
		switch (joint_type)
		{
			case JointType::NONE_JOINT:
			{
				...
				tQuaternion root_rot = btMathUtil::EulerAnglesToQuaternion(
					btMathUtil::Expand(q.segment(q_st, 3), 0), btRotationOrder::bt_XYZ);
...
/**
 * \brief				Convert char pose to the generalized coordinate q
*/
		...
		switch (cur_joint->GetJointType())
		{
			case JointType::NONE_JOINT:
			{
				...
				else
				{
					...
					// std::cout << "joint " << joint_id << "pose = " << pose.transpose() << std::endl;
					q.segment(q_st + 3, 3) = btMathUtil::QuaternionToEulerAngles(tQuaternion(w_x_y_z[0], w_x_y_z[1], w_x_y_z[2], w_x_y_z[3]), btRotationOrder::bt_XYZ).segment(0, 3);
					// std::cout << "joint " << joint_id << "pose = " << pose.transpose() << std::endl;
				}
			}
			break;
			case JointType::SPHERICAL_JOINT:
			{
				...
				else
				{
					...
					q.segment(q_st, 3) = btMathUtil::QuaternionToEulerAngles(tQuaternion(w_x_y_z[0], w_x_y_z[1], w_x_y_z[2], w_x_y_z[3]), btRotationOrder::bt_XYZ).segment(0, 3);
				}
			}
			break;
			case JointType::REVOLUTE_JOINT:
			...
			case JointType::FIXED_JOINT:
				...
				break;
			default:
				std::cout << "joint id " << joint_id << " type " << cur_joint->GetJointType() << std::endl;
				break;
		}
...
bool btTraj::LoadTraj(const std::string& path, cRobotModelDynamics* model, int max_frame /* = -1*/)
{
	...
	if (max_frame != -1) mNumOfFrames = std::min(max_frame, mNumOfFrames);
	...
	for (auto& x : mContactForce) x.clear();

	for (int frame_id = 0; frame_id < mNumOfFrames; frame_id++)
	{
		Json::Value cur_data = data_lst[frame_id];

		Json::Value char_pose_value = btJsonUtil::ParseAsValue("char_pose", cur_data);
		mTimestep = btJsonUtil::ParseAsDouble("timestep", cur_data);
		// std::cout << "dt = " << dt << std::endl;
		Json::Value action_value = btJsonUtil::ParseAsValue("truth_action", cur_data);
		int num_of_contact = btJsonUtil::ParseAsInt("contact_num", cur_data);
		Json::Value contact_info_value = btJsonUtil::ParseAsValue("contact_info", cur_data);
		...
		// std::cout << "action = " << mActiveForce[frame_id].transpose() << std::endl;
		...
		for (int c_id = 0; c_id < num_of_contact; c_id++)
		{
			...
			int link_id = btJsonUtil::ParseAsInt("force_link_id", cur_contact_value_json) - 1;
			tVectorXd pos, force_value;
			btJsonUtil::ReadVectorJson(btJsonUtil::ParseAsValue("force_pos", cur_contact_value_json), pos);
			btJsonUtil::ReadVectorJson(btJsonUtil::ParseAsValue("force_value", cur_contact_value_json), force_value);
			bool is_self_collision = btJsonUtil::ParseAsBool("is_self_collision", cur_contact_value_json);
			// if (is_self_collision == true)
			// {
			// 	std::cout << "[error] btGenContactAdviser: self collision hasn't been supported\n";
			// 	exit(1);
			// }
			btGenCollisionObject* obj = model->GetLinkCollider(link_id);
			// std::cout << "contact " << c_id << "link id " << link_id << " pos " << pos.transpose() << " force " << force_value.transpose() << std::endl;
			cur_contact_value = new btGenContactForce(obj, force_value.segment(0, 4), pos.segment(0, 4), is_self_collision);
		}
		// calculate
	}
	...
	for (int frame_id = 1; frame_id < mNumOfFrames - 1; frame_id++)
	{
		...
		mqddot[frame_id] = (-2 * mq[frame_id] + mq[frame_id - 1] + mq[frame_id + 1]) / (mTimestep * mTimestep);
		// std::cout << "mq = " << mq[frame_id].transpose() << std::endl;
		// std::cout << "mqdot = " << mqdot[frame_id].transpose() << std::endl;
		// std::cout << "mqddot = " << mqddot[frame_id].transpose() << std::endl;
		// exit(1);
	}
	return true;
}
...
bool btTraj::SaveTraj(const std::string& path, cRobotModelDynamics* model)
{
	...
	for (int i = 0; i < mNumOfFrames; i++)
	{
		...
		data_perframe["char_pose"] = btJsonUtil::BuildVectorJsonValue(ConvertqToPose(mq[i], model));
		data_perframe["timestep"] = this->mTimestep;
		data_perframe["truth_action"] = btJsonUtil::BuildVectorJsonValue(this->mActiveForce[i]);
		...
		for (auto& f : mContactForce[i])
		{
			...
			contact_info["force_pos"] = btJsonUtil::BuildVectorJsonValue(f->mWorldPos);
			contact_info["force_value"] = btJsonUtil::BuildVectorJsonValue(f->mForce);
